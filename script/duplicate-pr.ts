...
import { createCodeplane } from "@codeplane-ai/sdk"
...
`)
  ...
  const codeplane = await createCodeplane({ port: 0 })
  ...
  try {
    ...
    const session = await codeplane.client.session.create()
    const result = await codeplane.client.session
      .prompt({
