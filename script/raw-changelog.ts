...
const order = ["Core", "Web App", "SDK", "Extensions"] as const
const sections = {
  core: "Core",
  app: "Web App",
  sdk: "SDK",
...
function section(areas: Set<string>) {
  const priority = ["core", "app", "sdk", "plugin", "extensions/zed", "extensions/vscode", "github"]
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/codeplane packages/sdk packages/plugin packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/codeplane/")) areas.add("core")
