...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codeplane.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codeplane.settings.dat` is special: it maps to the `codeplane.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codeplane.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codeplane.settings.dat → the electron settings store ("codeplane.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codeplane.global.dat").
  const storeName = filename === "codeplane.settings.dat" ? "codeplane.settings" : filename
