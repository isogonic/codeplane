---
title: Server
description: Komunicirajte s codeplane serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codeplane serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codeplane klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codeplane.local` |
...
Postavite `CODEPLANE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codeplane`, ili postavite `CODEPLANE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codeplane serve` i za `codeplane web`.
...
Kada pokrenete `codeplane`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codeplane server za programsku interakciju sa codeplane-om.
:::

Ova arhitektura omogucava codeplane podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codeplane serve` da startate standalone server. Ako je codeplane TUI vec pokrenut, `codeplane serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CodePlane [IDE](/docs/ide) pluginovi.
...
codeplane server izlaže sljedece API-je.
