---
title: Konfiguracija
description: Korištenje CodePlane JSON konfiguracije.
---

Možete konfigurirati CodePlane koristeći JSON konfiguracijski fajl.

---

## Format

CodePlane podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/codeplane`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/codeplane/codeplane.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CODEPLANE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`codeplane.json` u projektu) - postavke specifične za projekat
5. **`.codeplane` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CODEPLANE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.codeplane` i `~/.config/codeplane` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/codeplane` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu CodePlane konfiguraciju u `~/.config/codeplane/codeplane.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/codeplane/tui.json`.
...
Dodajte `codeplane.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CodePlane pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CODEPLANE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODEPLANE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.codeplane` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.codeplane`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`codeplane.ai/config.json`**](https://codeplane.ai/config.json).

TUI konfiguracija koristi [**`codeplane.ai/tui.json`**](https://codeplane.ai/tui.json).
...
Koristite `CODEPLANE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `codeplane.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `codeplane serve` i `codeplane web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CodePlane server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `codeplane.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CodePlane konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CodePlane pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/codeplane/agents/` ili `.codeplane/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CodePlane će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`codeplane run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/codeplane/commands/` ili `.codeplane/commands/`. [Saznajte više ovdje](/docs/commands).
...
CodePlane će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CodePlane **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CodePlane sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.codeplane/plugins/` ili `~/.config/codeplane/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CodePlane da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
