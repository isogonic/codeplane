...
U CodePlane mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CodePlane koristi nasu `codeplane` temu.
...
CodePlane dolazi sa vise ugradenih tema.
...
- Zele da CodePlane odgovara izgledu njihovog terminala
...
CodePlane podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/codeplane/themes/*.json` ili `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.codeplane/themes/*.json`
4. **Trenutni radni direktorij** - `./.codeplane/themes/*.json`
