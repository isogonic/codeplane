---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

CodePlane koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CodePlane konfiguraciji.
...
u `~/.local/share/codeplane/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CodePlane
config.
...
CodePlane Zen je lista modela koje je obezbedio CodePlane tim koji su bili
testirano i potvrđeno da dobro radi sa CodePlane. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa CodePlane Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite codeplane i idite na [example.invalid/auth](https://example.invalid/auth).
...
Radi kao i svaki drugi provajder u CodePlane i potpuno je opcionalan za korištenje.

---

## CodePlane Go

CodePlane Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CodePlane tim i koji su testirani i verificirani da dobro rade s CodePlane-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `CodePlane Go` i idite na [example.invalid/auth](https://example.invalid/zen).
...
Da biste koristili Amazon Bedrock s CodePlane:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete codeplane:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codeplane.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CodePlane.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codeplane ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codeplane konfiguraciju.
...
da pohrani token u codeplane auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CodePlane koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CodePlane
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codeplane.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `codeplane.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s codeplane:
...
Za korištenje Google Vertex AI s CodePlane:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće codeplane.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem codeplanea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) da automatski prijavite svaki CodePlane razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CodePlane razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codeplane za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codeplane za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codeplane za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CodePlane. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/codeplane) za detalje.
:::
...
Da biste koristili Ollama Cloud s CodePlane:
...
3. Kopirajte API ključ za korištenje u CodePlane.
...
6. **Važno**: Prije upotrebe modela oblaka u CodePlane, morate lokalno povući informacije o modelu:
...
CodePlane Zen je lista testiranih i verifikovanih modela koju je obezbedio CodePlane tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>CodePlane Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **CodePlane Zen**.

```txt
   /connect
```

3. Unesite svoj CodePlane API ključ.
...
Također možete dodati dodatne modele putem vaše codeplane konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codeplane konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Codeplaneom:
...
Također možete prilagoditi modele kroz svoju codeplane konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s codeplane-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `codeplane.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CodePlane da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codeplane auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CodePlane konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codeplane konfiguraciji.
