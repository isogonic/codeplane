...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CodePlane dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CodePlane.
...
:::note
Ovaj alat je dostupan samo kada je `CODEPLANE_EXPERIMENTAL_LSP_TOOL=true` (ili `CODEPLANE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CodePlane provajdera ili kada je varijabla `CODEPLANE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CodePlane:
