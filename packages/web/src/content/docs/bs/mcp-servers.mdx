...
Možete dodati vanjske alate u CodePlane koristeći _Model Context Protocol_, ili MCP. CodePlane podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CodePlane Config](https://github.com/devinoldenburg/codeplane) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/codeplane`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
CodePlane automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CodePlane će:
...
Ako server zahtijeva autentifikaciju, CodePlane će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codeplane mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CodePlane sigurno čuva tokene u `~/.local/share/codeplane/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CodePlane, zajedno s ugrađenim alatima. Možete njima upravljati kroz CodePlane konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CodePlane sa vašim Sentry nalogom.
