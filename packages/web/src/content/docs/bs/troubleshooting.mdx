...
Da biste otklonili probleme s CodePlane, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codeplane\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `codeplane --log-level DEBUG`.

---

## Pohrana

CodePlane pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/codeplane/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codeplane`
...
CodePlane Desktop pokreće lokalni CodePlane server (`codeplane-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CodePlane` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/codeplane/codeplane.jsonc` (ili `~/.config/codeplane/codeplane.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/codeplane/codeplane.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codeplane\codeplane.jsonc`
...
CodePlane također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/codeplane/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codeplane\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CodePlane mogao ponovo izgraditi.

1. Potpuno zatvorite CodePlane Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/codeplane`
- **Linux**: obrišite `~/.cache/codeplane` (ili pokrenite `rm -rf ~/.cache/codeplane`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\codeplane`

3. Ponovo pokrenite CodePlane Desktop.

---

### Rješavanje problema sa vezom na serveru

CodePlane Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codeplane.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CODEPLANE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CODEPLANE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, CodePlane Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CodePlane karakteristikama.

---

### Obavještenja se ne prikazuju

CodePlane Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CodePlane u postavkama vašeg OS-a, i
...
1. Zatvorite CodePlane Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CodePlane Desktop aplikacije):

- `codeplane.settings.dat` (URL zadanog servera za desktop)
- `codeplane.global.dat` i `codeplane.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s CodePlane:
...
Da saznate kojim modelima imate pristup, pokrenite `codeplane models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\codeplane`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CodePlane dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\codeplane`

2. Ponovo pokrenite CodePlane da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CodePlane da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CodePlane će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
