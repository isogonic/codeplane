---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CodePlane.
---

Dodaci vam omogućavaju da proširite CodePlane spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CodePlane.
...
- `.codeplane/plugins/` - Dodaci na nivou projekta
- `~/.config/codeplane/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/codeplane/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/codeplane/codeplane.json`)
2. Konfiguracija projekta (`codeplane.json`)
3. Globalni direktorij dodataka (`~/.config/codeplane/plugins/`)
4. Direktorij dodataka projekta (`.codeplane/plugins/`)
...
CodePlane pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Codeplane SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CodePlane.
...
Spriječite codeplane da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u codeplane:
...
Pomoćnik `tool` kreira prilagođeni alat koji codeplane može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codeplane zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://github.com/devinoldenburg/codeplane) za detalje.
