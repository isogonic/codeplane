...
ove ili konfigurirajte svoje putem codeplane config.
...
codeplane dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.codeplane/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `codeplane.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/codeplane/modes/`
- Projekat: `.codeplane/modes/`
...
Ako temperatura nije navedena, codeplane koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.codeplane/modes/` za specifične načine rada ili `~/.config/codeplane/modes/` za globalne načine:
