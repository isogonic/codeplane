...
Postoje dvije vrste agenata u Codeplane: primarni agenti i podagenti.
...
Codeplane dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Codeplane dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Codeplane dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `codeplane.json`:
...
- Globalno: `~/.config/codeplane/agents/`
- Po projektu: `.codeplane/agents/`
...
Ako temperatura nije navedena, Codeplane koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Codeplane konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Codeplane konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Codeplane Zen](/docs/zen), koristili biste `codeplane/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `codeplane models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/devinoldenburg/codeplane).
:::
