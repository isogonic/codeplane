...
Agent skills omogucavaju Codeplane da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Codeplane pretrazuje ove lokacije:

- Konfiguracija projekta: `.codeplane/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/codeplane/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Codeplane ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codeplane/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.codeplane/skills/git-release/SKILL.md` ovako:
...
Codeplane navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codeplane.json`:
...
**Za ugradene agente** (u `codeplane.json`):
