---
title: Regeln
description: Lege eigene Anweisungen fuer codeplane fest.
---

Du kannst codeplane ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codeplane den Befehl `/init` aus.
...
So kann codeplane sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codeplane sie zu erweitern.
...
codeplane kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/codeplane/AGENTS.md` speichern.
Diese gelten in allen codeplane-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CodePlane dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/codeplane/AGENTS.md` existiert)
...
Beim Start sucht codeplane in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/codeplane/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codeplane.json` oder in `~/.config/codeplane/codeplane.json` angeben.
...
codeplane parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codeplane.json

Empfohlen ist das `instructions`-Feld in der `codeplane.json`:
...
Du kannst codeplane explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codeplane.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
