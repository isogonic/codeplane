---
title: Server
description: Nutze den codeplane-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `codeplane serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein codeplane-Client nutzen kann.
...
Setze `CODEPLANE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `codeplane`, kann aber mit `CODEPLANE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `codeplane serve` und `codeplane web`.
...
Wenn du `codeplane` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den codeplane-Server, um codeplane programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `codeplane serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `codeplane serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CodePlane-[IDE](/docs/ide)-Plugins.
...
Der codeplane-Server stellt folgende APIs bereit.
