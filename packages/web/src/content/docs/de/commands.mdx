...
Erstellen Sie `.codeplane/commands/test.md`:
...
Sie können benutzerdefinierte Befehle über die CodePlane-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem CodePlane [config](/docs/config):
...
- Pro Projekt: `.codeplane/commands/`
...
CodePlane enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
