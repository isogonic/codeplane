---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CodePlane zu erweitern.
---

Mit Plugins können Sie CodePlane erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CodePlane zu ändern.
...
- `.codeplane/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/codeplane/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/codeplane/codeplane.json`)
2. Projektkonfiguration (`codeplane.json`)
3. Globales Plugin-Verzeichnis (`~/.config/codeplane/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codeplane/plugins/`)
...
CodePlane führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CodePlane-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CodePlane erweitern können.
...
:::note
Wenn Sie die CodePlane-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CodePlane `.env`-Dateien liest:
...
Plugins können CodePlane auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CodePlane aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CodePlane zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://github.com/devinoldenburg/codeplane).
