...
Mit CodePlane kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet CodePlane das `codeplane`-Theme.
...
CodePlane liefert mehrere eingebaute Themes mit.
...
- CodePlane optisch ans Terminal anpassen wollen
...
CodePlane unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/codeplane/themes/*.json` oder `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.codeplane/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.codeplane/themes/*.json`
