---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

CodePlane verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer CodePlane-Konfiguration.
...
in `~/.local/share/codeplane/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem CodePlane anpassen
config.
...
CodePlane Zen ist eine Liste von Modellen, die vom CodePlane-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit CodePlane funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit CodePlane Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie CodePlane aus und gehen Sie zu [example.invalid/auth](https://example.invalid/auth).
...
Es funktioniert wie jeder andere Anbieter in CodePlane und ist völlig optional.

---

## CodePlane Go

CodePlane Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom CodePlane-Team getestet und verifiziert wurden, dass sie gut mit CodePlane funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `CodePlane Go` und gehen Sie zu [example.invalid/auth](https://example.invalid/zen).
...
So verwenden Sie Amazon Bedrock mit CodePlane:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie CodePlane ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `codeplane.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in CodePlane wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit CodePlane ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre CodePlane-Konfiguration hinzufügen.
...
um das Token im CodePlane-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
CodePlane verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von CodePlane
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`codeplane.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit CodePlane:
...
So verwenden Sie Google Vertex AI mit CodePlane:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie CodePlane ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über CodePlane konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session)-Plugin, um jede CodePlane-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede CodePlane-Konversation als separate Sitzung aufgeführt.
...
Sie können CodePlane für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können CodePlane über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können CodePlane über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für CodePlane konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/codeplane).
:::
...
So verwenden Sie Ollama Cloud mit CodePlane:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in CodePlane.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in CodePlane verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
CodePlane Zen ist eine Liste getesteter und verifizierter Modelle, die vom CodePlane-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>CodePlane Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren CodePlane API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre CodePlane-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre CodePlane-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit CodePlane:
...
Sie können Modelle auch über Ihre CodePlane-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit CodePlane verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `codeplane.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann CodePlane erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `codeplane auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die CodePlane-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer CodePlane-Konfiguration übereinstimmt.
