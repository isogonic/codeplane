...
:::caution
Modi werden jetzt über die Option `agent` in der Codeplane-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in Codeplane können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die Codeplane-Konfiguration.
...
Codeplane verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.codeplane/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `codeplane.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Codeplane modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale Codeplane-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.codeplane/modes/` für projektspezifische Modi oder `~/.config/codeplane/modes/` für globale Modi:
