...
Agent Skills erlauben Codeplane, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Codeplane sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Codeplane vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codeplane/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.codeplane/skills/git-release/SKILL.md` zum Beispiel so:
...
Codeplane listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codeplane.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codeplane.json`):
