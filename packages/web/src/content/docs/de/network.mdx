...
CodePlane supports standard proxy environment variables and custom certificates for enterprise network environments.

---

## Proxy

CodePlane respects standard proxy environment variables.
...
:::caution
The web app communicates with a local HTTP server. You must bypass the proxy for this connection to prevent routing loops.
:::
...
If your enterprise uses custom CAs for HTTPS connections, configure CodePlane to trust them.
