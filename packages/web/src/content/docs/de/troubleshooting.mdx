...
Wenn CodePlane Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codeplane\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `codeplane --log-level DEBUG`.

---

## Speicher

codeplane speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/codeplane/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codeplane`
...
CodePlane Desktop startet im Hintergrund einen lokalen CodePlane-Server (`codeplane-cli`-Sidecar).
...
- Nur macOS: `CodePlane`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/codeplane/codeplane.jsonc` (oder `~/.config/codeplane/codeplane.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/codeplane/codeplane.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codeplane\codeplane.jsonc`
...
CodePlane kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/codeplane/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codeplane\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CodePlane ihn neu aufbauen kann.

1. Beende CodePlane Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/codeplane`
- **Linux**: loesche `~/.cache/codeplane` (oder fuehre aus: `rm -rf ~/.cache/codeplane`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\codeplane`

3. Starte CodePlane Desktop neu.

---

### Server-Verbindungsprobleme beheben

CodePlane Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codeplane.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CODEPLANE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CODEPLANE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt CodePlane Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CodePlane-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CodePlane Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CodePlane in den OS-Einstellungen aktiviert sind, und
...
1. Beende CodePlane Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CodePlane Desktop):

- `codeplane.settings.dat` (desktop default server URL)
- `codeplane.global.dat` und `codeplane.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit CodePlane hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codeplane models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\codeplane`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codeplane installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\codeplane`

2. Starte codeplane neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt codeplane, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
codeplane erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
