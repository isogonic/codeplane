...
With Codeplane you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Codeplane uses our own `codeplane` theme.
...
Codeplane comes with several built-in themes.
...
- Want Codeplane to match their terminal's appearance
...
You can select a theme from Settings -> General -> Theme in the web app.

---

## Custom themes

Codeplane supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/codeplane/themes/*.json` or `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.codeplane/themes/*.json`
4. **Current working directory** - Defined in `./.codeplane/themes/*.json`
