...
Le skill degli agenti permettono a Codeplane di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Codeplane cerca in queste posizioni:

- Config di progetto: `.codeplane/skills/<name>/SKILL.md`
- Config globale: `~/.config/codeplane/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Codeplane risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codeplane/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.codeplane/skills/git-release/SKILL.md` cosi':
...
Codeplane elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codeplane.json`:
...
**Per agenti integrati** (in `codeplane.json`):
