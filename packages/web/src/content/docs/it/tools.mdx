...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. CodePlane include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in CodePlane.
...
:::note
Questo strumento e' disponibile solo quando `CODEPLANE_EXPERIMENTAL_LSP_TOOL=true` (oppure `CODEPLANE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider CodePlane o quando la variabile d'ambiente `CODEPLANE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di CodePlane:
