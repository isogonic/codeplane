...
Crea `.codeplane/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di CodePlane o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di CodePlane:
...
- Globale: `~/.config/codeplane/commands/`
- Per progetto: `.codeplane/commands/`
...
codeplane include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
