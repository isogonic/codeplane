...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di codeplane. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in codeplane ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di codeplane.
...
codeplane include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.codeplane/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `codeplane.json`:
...
- Globale: `~/.config/codeplane/modes/`
- Progetto: `.codeplane/modes/`
...
Se non viene specificata alcuna temperatura, codeplane usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di codeplane sia per quella specifica del progetto.
...
Crea file di modalita in `.codeplane/modes/` per modalita specifiche del progetto oppure in `~/.config/codeplane/modes/` per modalita globali:
