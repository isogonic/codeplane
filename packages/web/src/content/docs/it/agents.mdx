...
In CodePlane esistono due tipi di agenti: agenti primari e subagenti.
...
CodePlane include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
CodePlane include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

CodePlane include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `codeplane.json`:
...
- Globale: `~/.config/codeplane/agents/`
- Per progetto: `.codeplane/agents/`
...
Se non specifichi una temperature, CodePlane usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di CodePlane sia per la config specifica del progetto.
...
L'ID modello nella configurazione CodePlane usa il formato `provider/model-id`. Per esempio, se stai usando [CodePlane Zen](/docs/zen), useresti `codeplane/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `codeplane models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/devinoldenburg/codeplane).
:::
