...
CodePlane Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
CodePlane Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in CodePlane. Ti abboni a CodePlane Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare CodePlane.
...
CodePlane Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

CodePlane Go funziona come qualsiasi altro provider in CodePlane.

1. Accedi a **<a href={console}>CodePlane Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `CodePlane Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a CodePlane Go.
:::
...
CodePlane Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua CodePlane config
utilizza il formato `codeplane-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`codeplane-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato CodePlane Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con CodePlane.
