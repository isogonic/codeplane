---
title: Server
description: Interagisci con il server codeplane via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `codeplane serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client codeplane.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `codeplane.local` |
...
Imposta `CODEPLANE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `codeplane`, oppure imposta `CODEPLANE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `codeplane serve` sia per `codeplane web`.
...
Quando esegui `codeplane` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server codeplane per interagire con codeplane in modo programmatico.
:::

Questa architettura permette a codeplane di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `codeplane serve` per avviare un server standalone. Se la TUI di codeplane e gia in esecuzione, `codeplane serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di CodePlane.
...
Il server codeplane espone le seguenti API.
