---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CodePlane.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodePlane fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CodePlane avvia la TUI per la directory corrente.
...
Quando usi la TUI di CodePlane, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CodePlane. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CodePlane. _Alias_: `/quit`, `/q`
...
Questo è separato da `codeplane.json`, che configura il comportamento di server/runtime.
...
Usa `CODEPLANE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
