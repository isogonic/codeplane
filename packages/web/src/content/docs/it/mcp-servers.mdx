...
Puoi aggiungere strumenti esterni a CodePlane usando il _Model Context Protocol_, o MCP. CodePlane supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CodePlane Config](https://github.com/devinoldenburg/codeplane) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/codeplane`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
CodePlane gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CodePlane:
...
Se il server richiede autenticazione, CodePlane ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `codeplane mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CodePlane memorizzera' i token in modo sicuro in `~/.local/share/codeplane/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CodePlane insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CodePlane come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CodePlane al tuo account Sentry.
