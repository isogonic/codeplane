---
title: Plugin
description: Scrivi plugin per estendere CodePlane.
---

I plugin ti permettono di estendere CodePlane agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CodePlane.
...
- `.codeplane/plugins/` - plugin a livello progetto
- `~/.config/codeplane/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/codeplane/node_modules/`.
...
1. Config globale (`~/.config/codeplane/codeplane.json`)
2. Config di progetto (`codeplane.json`)
3. Directory plugin globale (`~/.config/codeplane/plugins/`)
4. Directory plugin di progetto (`.codeplane/plugins/`)
...
CodePlane esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codeplane per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere codeplane.
...
:::note
Se usi l'app desktop di CodePlane, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codeplane di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a codeplane:
...
L'helper `tool` crea uno strumento personalizzato che codeplane può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codeplane insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/devinoldenburg/codeplane) per i dettagli.
