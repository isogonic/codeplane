---
title: Plugins
description: Escreva seus próprios plugins para estender o codeplane.
---

Plugins permitem que você estenda o codeplane conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do codeplane.
...
- `.codeplane/plugins/` - Plugins em nível de projeto
- `~/.config/codeplane/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/codeplane/node_modules/`.
...
1. Configuração global (`~/.config/codeplane/codeplane.json`)
2. Configuração do projeto (`codeplane.json`)
3. Diretório de plugins global (`~/.config/codeplane/plugins/`)
4. Diretório de plugins do projeto (`.codeplane/plugins/`)
...
O codeplane executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codeplane para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o codeplane.
...
:::note
Se você estiver usando o aplicativo desktop codeplane, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o codeplane de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao codeplane:
...
O helper `tool` cria uma ferramenta personalizada que o codeplane pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codeplane junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://github.com/devinoldenburg/codeplane) para detalhes.
