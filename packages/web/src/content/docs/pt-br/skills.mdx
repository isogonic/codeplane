...
As habilidades do agente permitem que o codeplane descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codeplane pesquisa nesses locais:

- Configuração do projeto: `.codeplane/skills/<name>/SKILL.md`
- Configuração global: `~/.config/codeplane/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codeplane sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codeplane/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.codeplane/skills/git-release/SKILL.md` assim:
...
O codeplane lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codeplane.json`:
...
**Para agentes embutidos** (em `codeplane.json`):
