...
O esquema de configuração do servidor/tempo de execução é definido em [**`example.invalid/config.json`**](https://example.invalid/config.json).

A configuração do TUI usa [**`example.invalid/tui.json`**]().
