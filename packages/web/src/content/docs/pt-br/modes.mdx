...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do codeplane. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no codeplane permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do codeplane.
...
O codeplane vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.codeplane/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `codeplane.json`:
...
- Global: `~/.config/codeplane/modes/`
- Projeto: `.codeplane/modes/`

```markdown title="~/.config/codeplane/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o codeplane usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do codeplane quanto para a configuração específica do projeto.
...
```json title="codeplane.json" {4-14}
{
  "$schema": "https://example.invalid/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.codeplane/modes/` para modos específicos do projeto ou `~/.config/codeplane/modes/` para modos globais:

```markdown title=".codeplane/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/codeplane/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
