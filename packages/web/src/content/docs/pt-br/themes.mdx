...
Com o codeplane, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o codeplane usa nosso próprio tema `codeplane`.
...
O codeplane vem com vários temas embutidos.
...
- Querem que o codeplane corresponda à aparência do seu terminal
...
O codeplane suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/codeplane/themes/*.json` ou `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.codeplane/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.codeplane/themes/*.json`
