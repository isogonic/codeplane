---
title: Servidor
description: Interaja com o servidor codeplane via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codeplane serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codeplane pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codeplane.local` |
...
Defina `CODEPLANE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codeplane`, ou defina `CODEPLANE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codeplane serve` quanto ao `codeplane web`.

```bash
CODEPLANE_SERVER_PASSWORD=your-password codeplane serve
```
...
Quando você executa `codeplane`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codeplane para interagir com o codeplane programaticamente.
:::

Essa arquitetura permite que o codeplane suporte múltiplos clientes e permite que você interaja com o codeplane programaticamente.

Você pode executar `codeplane serve` para iniciar um servidor autônomo. Se você tiver o TUI do codeplane em execução, `codeplane serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do codeplane [IDE](/docs/ide).
...
O servidor codeplane expõe as seguintes APIs.
