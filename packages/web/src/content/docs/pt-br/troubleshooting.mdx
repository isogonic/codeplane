...
Para depurar problemas com o codeplane, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\codeplane\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `codeplane --log-level DEBUG`.

---

## Armazenamento

O codeplane armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/codeplane/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\codeplane`
...
O codeplane Desktop executa um servidor codeplane local (o sidecar `codeplane-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `CodePlane` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/codeplane/codeplane.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\codeplane\codeplane.jsonc`
...
O codeplane também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/codeplane/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\codeplane\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o codeplane possa reconstruí-lo.

1. Saia completamente do codeplane Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/codeplane`
- **Linux**: exclua `~/.cache/codeplane` (ou execute `rm -rf ~/.cache/codeplane`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\codeplane`

3. Reinicie o codeplane Desktop.

---

### Corrigindo problemas de conexão com o servidor

O codeplane Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `codeplane.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `CODEPLANE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `CODEPLANE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o codeplane Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do codeplane.

---

### Notificações não aparecendo

O codeplane Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o codeplane nas configurações do seu sistema operacional, e
...
1. Saia do codeplane Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo codeplane Desktop):

- `codeplane.settings.dat` (URL do servidor padrão do desktop)
- `codeplane.global.dat` e `codeplane.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o codeplane:
...
Para descobrir quais modelos você tem acesso, execute `codeplane models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\codeplane`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O codeplane instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\codeplane`

2. Reinicie o codeplane para reinstalar os pacotes de provedores mais recentes

Isso forçará o codeplane a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O codeplane detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
