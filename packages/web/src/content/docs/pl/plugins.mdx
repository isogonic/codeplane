---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć codeplane.
---

Wtyczki umożliwiające wyłączenie codeplane poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania codeplane.
...
- `.codeplane/plugins/` - Wtyczki na poziomie projektu
- `~/.config/codeplane/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/codeplane/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/codeplane/codeplane.json`)
2. Konfiguracja projektu (`codeplane.json`)
3. Globalny katalog wtyczek (`~/.config/codeplane/plugins/`)
4. Katalog wtyczek projektu (`.codeplane/plugins/`)
...
codeplane uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z codeplane do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do codeplane.
...
:::note
Jeśli korzystasz z aplikacji komputerowej codeplane, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj codeplane czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do codeplane:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod codeplane. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://github.com/devinoldenburg/codeplane).
