...
Możesz dodać dodatkowe informacje do codeplane za pomocą _Model Context Protocol_, w skrócie MCP. codeplane obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [codeplane Config](https://github.com/devinoldenburg/codeplane) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/codeplane`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
codeplane automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, codeplane:
...
Jeśli serwer wymaga uwierzytelnienia, codeplane poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codeplane mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codeplane bezpieczne przechowa tokeny w `~/.local/share/codeplane/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w codeplane, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie codeplane, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codeplane z kontem Sentry.
