...
Dzięki codeplane możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codeplane używa naszego własnego motywu `codeplane`.
...
codeplane ma kilka wbudowanych motywów.
...
- Chcą, aby CodePlane pasował do wyglądu ich terminala
...
codeplane obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/codeplane/themes/*.json` lub `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codeplane/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codeplane/themes/*.json`
