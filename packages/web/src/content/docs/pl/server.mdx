---
title: Serwer
description: Interakcja z serwerem codeplane za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codeplane serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient codeplane.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codeplane.local` |
...
Ustaw `CODEPLANE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codeplane` lub ustaw `CODEPLANE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codeplane serve`, jak i `codeplane web`.
...
Kiedy uruchomisz `codeplane`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codeplane do programowej interakcji z codeplane.
:::

Ta architektura umożliwia obsługę wielu klientów przez codeplane i programową interakcję z kodem otwartym.

Możesz uruchomić `codeplane serve`, aby uruchomić samodzielny serwer. Jeśli masz
codeplane TUI działa, `codeplane serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki codeplane [IDE](/docs/ide).
...
Serwer codeplane udostępnia następujące interfejsy API.
