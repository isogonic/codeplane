---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w codeplane.
---

import config from "../../../../config.mjs"
export const console = config.console

codeplane używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji codeplane.
...
w `~/.local/share/codeplane/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim codeplane
config.
...
CodePlane Zen to lista modeli dostarczonych przez zespół codeplane, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z codeplane. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od CodePlane Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `CodePlane Zen` i przejdź do [example.invalid/auth](https://example.invalid/zen).
...
Działa jak każdy inny dostawca w codeplane i jest całkowicie opcjonalny w użyciu.

---

## CodePlane Go

CodePlane Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół codeplane, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z codeplane.

1. Uruchom polecenie `/connect` w TUI, wybierz `CodePlane Go` i przejdź do [example.invalid/auth](https://example.invalid/zen).
...
Aby używać Amazon Bedrock z codeplane:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania codeplane:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codeplane.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codeplane nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codeplane działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji codeplane.
...
codeplane używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować codeplane
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`codeplane.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem codeplane:
...
Aby używać Google Vertex AI z codeplane:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania codeplane.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą codeplane, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session), aby automatycznie rejestrować każdą konwersację codeplane jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codeplane będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codeplane tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codeplane tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codeplane tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem codeplane. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/codeplane).
:::
...
Aby korzystać z Ollama Cloud z codeplane:
...
3. Skopiuj klucz API do użycia w codeplane.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codeplane musisz pobrać informacje o modelu lokalnie:
...
CodePlane Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół codeplane. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>CodePlane Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API codeplane.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji codeplane.
...
5. Możesz je także dostosować za pomocą konfiguracji codeplane. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z codeplane:
...
Możesz także dostosować modele za pomocą konfiguracji codeplane. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z codeplane. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `codeplane.json` w katalogu projektu:
...
Pola `limit` pozwalają codeplane zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codeplane auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codeplane i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji codeplane.
