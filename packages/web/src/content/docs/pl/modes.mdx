...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji codeplane.
...
te lub skonfiguruj własne za pomocą konfiguracji codeplane.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.codeplane/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `codeplane.json`:
...
- Globalnie: `~/.config/codeplane/modes/`
...
Jeśli nie ma wpływu na temperaturę, codeplane stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna codeplane, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.codeplane/modes/` dla trybów zapisanych dla projektu lub `~/.config/codeplane/modes/` dla trybów globalnych:
