...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CodePlane posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CodePlane.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CODEPLANE_EXPERIMENTAL_LSP_TOOL=true` (lub `CODEPLANE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CodePlane lub gdy zmienna środowiskowa `CODEPLANE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CodePlane:
