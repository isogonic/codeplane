...
Umiejętności agenta pozwalają codeplane odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codeplane przeszukuje te lokalizacje:

- Project config: `.codeplane/skills/<name>/SKILL.md`
- Global config: `~/.config/codeplane/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codeplane przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codeplane/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.codeplane/skills/git-release/SKILL.md` w ten sposób:
...
codeplane wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codeplane.json`:
...
**Dla agentów wbudowanych** (w `codeplane.json`):
