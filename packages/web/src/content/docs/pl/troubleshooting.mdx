...
Aby debugować problemy z codeplane, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\codeplane\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `codeplane --log-level DEBUG`.

---

## Przechowywanie danych

codeplane przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/codeplane/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\codeplane`
...
codeplane Desktop uruchamia w tle lokalny serwer codeplane (sidecar `codeplane-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `CodePlane` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/codeplane/codeplane.jsonc` (lub `~/.config/codeplane/codeplane.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/codeplane/codeplane.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\codeplane\codeplane.jsonc`
...
codeplane może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/codeplane/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\codeplane\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby codeplane mógł ją odbudować.

1. Całkowicie zamknij codeplane Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/codeplane`
- **Linux**: usuń `~/.cache/codeplane` (lub uruchom `rm -rf ~/.cache/codeplane`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\codeplane`

3. Uruchom ponownie codeplane Desktop.

---

### Napraw problemy z połączeniem z serwerem

codeplane Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `codeplane.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CODEPLANE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CODEPLANE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows codeplane Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami codeplane.

---

### Brak powiadomień

codeplane Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla codeplane w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie codeplane Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji codeplane Desktop):

- `codeplane.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `codeplane.global.dat` i `codeplane.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z codeplane:
...
3. Upewnij się, że masz najnowszą wersję z `codeplane upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `codeplane models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\codeplane`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. codeplane dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\codeplane`

2. Uruchom ponownie kod codeplane, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to codeplane do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
codeplane wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
