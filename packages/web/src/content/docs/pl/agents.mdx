...
W Codeplane obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Codeplane zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Codeplane ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Codeplane ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `codeplane.json`:
...
- Globalnie: `~/.config/codeplane/agents/`
- W projekcie: `.codeplane/agents/`
...
Jeśli nie zostanie skonfigurowana, Codeplane używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Codeplane, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Codeplane w `provider/model-id`. Na przykład, [Codeplane Zen](/docs/zen), użyj `codeplane/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `codeplane models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/devinoldenburg/codeplane).
:::
