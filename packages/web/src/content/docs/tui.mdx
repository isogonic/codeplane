---
title: TUI
description: Using the CodePlane terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodePlane provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CodePlane starts the TUI for the current directory.
...
When using the CodePlane TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CodePlane. Allows you to select from available providers and add their API keys.
...
Exit CodePlane. _Aliases_: `/quit`, `/q`
...
This is separate from `codeplane.json`, which configures server/runtime behavior.
...
Use `CODEPLANE_TUI_CONFIG` to load a custom TUI config path.
