---
title: الخادم
description: تفاعل مع خادم codeplane عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `codeplane serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل codeplane استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `codeplane.local` |
...
عيّن `CODEPLANE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `codeplane`، أو عيّن `CODEPLANE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `codeplane serve` و `codeplane web`.
...
عند تشغيل `codeplane` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم codeplane للتفاعل مع codeplane برمجيا.
:::

تتيح هذه البنية لـ codeplane دعم عدة عملاء وتمكّنك من التفاعل مع codeplane برمجيا.

يمكنك تشغيل `codeplane serve` لبدء خادم مستقل. إذا كانت واجهة codeplane في terminal (TUI)
قيد التشغيل، فسيبدأ `codeplane serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CodePlane لـ [IDE](/docs/ide).
...
يعرض خادم codeplane واجهات API التالية.
