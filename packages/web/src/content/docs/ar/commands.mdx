...
أنشئ الملف `.codeplane/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات CodePlane أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ CodePlane:
...
- على مستوى النظام: `~/.config/codeplane/commands/`
- لكل مشروع: `.codeplane/commands/`
...
يتضمن codeplane عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
