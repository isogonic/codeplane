...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات codeplane. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في codeplane تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات codeplane.
...
يأتي codeplane مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.codeplane/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `codeplane.json` لديك:
...
- عالمي: `~/.config/codeplane/modes/`
- للمشروع: `.codeplane/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم codeplane القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات codeplane العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.codeplane/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/codeplane/modes/` لأوضاع عالمية:
