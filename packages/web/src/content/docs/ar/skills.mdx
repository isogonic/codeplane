...
تتيح مهارات الوكيل لـ Codeplane اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Codeplane في هذه المواقع:

- إعدادات المشروع: `.codeplane/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/codeplane/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Codeplane صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.codeplane/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/codeplane/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.codeplane/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Codeplane المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `codeplane.json`:
...
**للوكلاء المدمجين** (في `codeplane.json`):
