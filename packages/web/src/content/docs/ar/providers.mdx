---
title: المزوّدون
description: استخدام أي موفّر LLM في CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CodePlane ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CodePlane.
...
في `~/.local/share/codeplane/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CodePlane.
...
CodePlane Zen هي قائمة نماذج يوفّرها فريق CodePlane وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CodePlane. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CodePlane Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر codeplane، ثم انتقل إلى [example.invalid/auth](https://example.invalid/auth).
...
يعمل مثل أي مزوّد آخر في CodePlane واستخدامه اختياري بالكامل.

---

## CodePlane Go

CodePlane Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CodePlane، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CodePlane.

1. شغّل الأمر `/connect` في TUI، واختر `CodePlane Go`، ثم انتقل إلى [example.invalid/auth](https://example.invalid/zen).
...
لاستخدام Amazon Bedrock مع CodePlane:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل codeplane:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codeplane.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CodePlane غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل codeplane بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات codeplane.
...
تخزين الرمز في مساحة تخزين مصادقة codeplane.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم CodePlane نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CodePlane
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codeplane.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع codeplane:
...
لاستخدام Google Vertex AI مع CodePlane:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر codeplane، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) لتسجيل كل محادثة CodePlane تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CodePlane مدرجة كجلسة مستقلة.
...
يمكنك ضبط codeplane لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط codeplane لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط codeplane لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CodePlane. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/codeplane) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CodePlane:
...
3. انسخ مفتاح API لاستخدامه في CodePlane.
...
6. **مهم**: قبل استخدام نماذج السحابة في CodePlane، يجب سحب معلومات النموذج محليا:
...
CodePlane Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CodePlane. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CodePlane Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CodePlane Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CodePlane.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات codeplane.
...
5. يمكنك أيضا تخصيصها عبر إعدادات codeplane. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Codeplane:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات codeplane. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع codeplane. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `codeplane.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ CodePlane بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codeplane auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات codeplane وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات codeplane.
