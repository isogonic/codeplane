...
يوجد نوعان من الوكلاء في CodePlane: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي CodePlane مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي CodePlane مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي CodePlane مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
هيّئ الوكلاء في ملف إعدادات `codeplane.json`:
...
- عام: `~/.config/codeplane/agents/`
- لكل مشروع: `.codeplane/agents/`
...
إذا لم يتم تحديد temperature، يستخدم CodePlane القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات CodePlane العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات CodePlane الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [CodePlane Zen](/docs/zen)، فستستخدم `codeplane/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `codeplane models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/devinoldenburg/codeplane).
:::
