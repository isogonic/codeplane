...
مع CodePlane يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم CodePlane سمتنا `codeplane`.
...
يأتي CodePlane مع عدة سمات مدمجة.
...
- يريدون أن يطابق CodePlane مظهر terminal لديهم
...
يدعم CodePlane نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/codeplane/themes/*.json` أو `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.codeplane/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.codeplane/themes/*.json`
