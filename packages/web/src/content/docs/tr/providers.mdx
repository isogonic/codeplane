---
title: Sağlayıcılar
description: codeplane'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

codeplane, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı codeplane yapılandırmanızda yapılandırın.
...
`~/.local/share/codeplane/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları codeplane'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
CodePlane Zen, codeplane ekibi tarafından sağlanan modellerin bir listesidir.
codeplane ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz CodePlane Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, codeplane'u seçin ve [example.invalid/auth](https://example.invalid/auth)'ye gidin.
...
codeplane'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## CodePlane Go

CodePlane Go, codeplane ile iyi çalıştığı test edilmiş ve doğrulanmış, codeplane ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `CodePlane Go`'yu seçin ve [example.invalid/auth](https://example.invalid/zen) adresine gidin.
...
Amazon Bedrock'u codeplane ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   codeplane'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `codeplane.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi codeplane'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   codeplane'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Codeplane yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in codeplane auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
codeplane, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. codeplane'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`codeplane.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`codeplane.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi codeplane'la kullanmak için:
...
Google Vertex AI'yi codeplane ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları codeplane çalıştırılırken ayarlayın.
...
Helicone'un codeplane aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her codeplane görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her codeplane görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
codeplane'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla codeplane'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla codeplane'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini codeplane için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/codeplane)'a bakın.
:::
...
Ollama Cloud'u codeplane ile kullanmak için:
...
3. codeplane'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: codeplane'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
CodePlane Zen, codeplane ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>CodePlane Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **CodePlane Zen**'i arayın.

   ```txt
   /connect
   ```

3. codeplane API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   codeplane yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca codeplane yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Codeplane ile kullanmak için:
...
Modelleri codeplane yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı codeplane ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `codeplane.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları codeplane'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `codeplane auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için codeplane yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, codeplane yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
