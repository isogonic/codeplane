...
description: codeplane dile özgü biçimlendiriciler kullanır.
---

codeplane, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

codeplane, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, codeplane bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

codeplane bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri codeplane yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
