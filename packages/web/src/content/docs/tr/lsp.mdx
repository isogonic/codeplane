...
description: CodePlane, LSP sunucularınızla bütünleşir.
---

CodePlane, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

CodePlane, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CODEPLANE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
codeplane bir dosyayı açtığında:
...
LSP sunucularını, codeplane yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
