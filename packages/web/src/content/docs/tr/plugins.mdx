---
title: Eklentiler
description: CodePlane'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CodePlane'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CodePlane'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codeplane/plugins/` - Proje düzeyinde eklentiler
- `~/.config/codeplane/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/codeplane/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/codeplane/codeplane.json`)
2. Project config (`codeplane.json`)
3. Global eklenti dizini (`~/.config/codeplane/plugins/`)
4. Proje eklenti dizini (`.codeplane/plugins/`)
...
CodePlane bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CodePlane SDK istemcisi.
...
CodePlane'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CodePlane masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CodePlane'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CodePlane'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CodePlane'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CodePlane'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://github.com/devinoldenburg/codeplane) bakın.
