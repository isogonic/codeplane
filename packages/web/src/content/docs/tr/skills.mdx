...
Ajan becerileri, codeplane'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
codeplane su konumlari tarar:

- Proje konfigurasyonu: `.codeplane/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/codeplane/skills/<name>/SKILL.md`
...
Proje yerel yollarinda codeplane, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.codeplane/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.codeplane/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
codeplane, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`codeplane.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`codeplane.json` icinde):
