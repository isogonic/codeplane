...
CodePlane Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
CodePlane Go şu anda beta olarak mevcut.
:::

Go, CodePlane'daki diğer sağlayıcılar gibi çalışır. CodePlane Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CodePlane'u kullanmak için bunu kullanmanıza gerek yoktur.
...
CodePlane Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

CodePlane Go, CodePlane'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>CodePlane Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `CodePlane Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye CodePlane Go'ya abone olabilir.
:::
...
CodePlane Go aşağıdaki limitleri içerir:
...
CodePlane yapılandırmanızdaki [model id](/docs/config/#models) formatı `codeplane-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `codeplane-go/kimi-k2.6` kullanmalısınız.
...
CodePlane Go'yu şu amaçlarla oluşturduk:
...
4. CodePlane ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
