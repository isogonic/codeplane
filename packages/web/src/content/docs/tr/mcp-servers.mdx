...
_Model Bağlam Protokolü_ veya MCP kullanarak codeplane'a harici araçlar ekleyebilirsiniz. codeplane hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [codeplane Config](https://github.com/devinoldenburg/codeplane)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/codeplane` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
codeplane, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde codeplane şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa codeplane, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codeplane mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından codeplane, belirteçleri `~/.local/share/codeplane/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra codeplane'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codeplane yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve codeplane'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
