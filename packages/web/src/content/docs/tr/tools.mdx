...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. codeplane yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
codeplane içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CODEPLANE_EXPERIMENTAL_LSP_TOOL=true` (veya `CODEPLANE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca codeplane provider kullanılırken veya `CODEPLANE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

codeplane başlatırken etkinleştirmek için:
