...
codeplane ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codeplane\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `codeplane --log-level DEBUG`.

---

## Depolama

codeplane, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/codeplane/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codeplane` yapistirin
...
codeplane Desktop arka planda yerel bir codeplane sunucusu (`codeplane-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `codeplane` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/codeplane/codeplane.jsonc` (veya `~/.config/codeplane/codeplane.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/codeplane/codeplane.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codeplane\codeplane.jsonc` yapistirin
...
codeplane diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/codeplane/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codeplane\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), codeplane'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. codeplane Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/codeplane`
- **Linux**: `~/.cache/codeplane` dizinini silin (veya `rm -rf ~/.cache/codeplane` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\codeplane` yapistirin

3. codeplane Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

codeplane Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`codeplane.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `CODEPLANE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `CODEPLANE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta codeplane Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, codeplane ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

codeplane Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda codeplane icin bildirimler etkinse
...
1. codeplane Desktop'u kapatin
2. Su dosyalari bulun ve silin (codeplane Desktop uygulama veri dizininde yer alirlar):

- `codeplane.settings.dat` (desktop varsayilan sunucu URL'si)
- `codeplane.global.dat` ve `codeplane.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
codeplane ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `codeplane models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\codeplane`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. codeplane, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\codeplane`

2. En guncel provider paketlerini yeniden kurmak icin codeplane'u yeniden baslatin

Bu, codeplane'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
codeplane, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
