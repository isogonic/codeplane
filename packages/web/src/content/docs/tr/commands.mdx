...
`.codeplane/commands/test.md` oluştur:
...
codeplane işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

codeplane [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/codeplane/commands/`
- Proje başına: `.codeplane/commands/`
...
codeplane, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
