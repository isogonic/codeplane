...
codeplane ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak codeplane kendi `codeplane` temasını kullanır.
...
codeplane birden fazla yerleşik temayla gelir.
...
- codeplane'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
codeplane, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/codeplane/themes/*.json` veya `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.codeplane/themes/*.json`
4. **Geçerli çalışma dizini** - `./.codeplane/themes/*.json`
