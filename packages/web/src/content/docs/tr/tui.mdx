---
title: TUI
description: CodePlane terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodePlane, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

CodePlane'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
CodePlane TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
CodePlane'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
CodePlane'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `codeplane.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `CODEPLANE_TUI_CONFIG` kullanın.
