...
codeplane'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
codeplane, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
codeplane, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

codeplane iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `codeplane.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse codeplane modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global codeplane yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
codeplane yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [CodePlane Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `codeplane/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `codeplane models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/devinoldenburg/codeplane).
:::
