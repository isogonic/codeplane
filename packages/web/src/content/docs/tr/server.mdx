---
title: Sunucu
description: codeplane sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codeplane serve` komutu, codeplane istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `codeplane.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CODEPLANE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `codeplane` değeridir; değiştirmek isterseniz `CODEPLANE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `codeplane serve` hem de `codeplane web` için geçerlidir.
...
`codeplane` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
codeplane ile programatik etkileşim için codeplane sunucusunu kullanın.
:::

Bu mimari, codeplane'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`codeplane serve` ile bağımsız bir sunucu başlatabilirsiniz. codeplane TUI açıksa, `codeplane serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum codeplane [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
codeplane sunucusu aşağıdaki API'leri sunar.
