...
:::caution
Modlar artık codeplane yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

codeplane'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya codeplane yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
codeplane iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.codeplane/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`codeplane.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, codeplane modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global codeplane yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.codeplane/modes/` veya genel modlar için `~/.config/codeplane/modes/`'de mod dosyaları oluşturun:
