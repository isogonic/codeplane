...
Создайте `.codeplane/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию codeplane или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем codeplane [config](/docs/config):
...
- Глобальный: `~/.config/codeplane/commands/`
- Для каждого проекта: `.codeplane/commands/`
...
codeplane включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
