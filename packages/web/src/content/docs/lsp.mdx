---
title: LSP Servers
description: CodePlane integrates with your LSP servers.
---

CodePlane integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

CodePlane comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CODEPLANE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When codeplane opens a file, it:
...
You can customize LSP servers through the `lsp` section in your codeplane config.
