...
Agentferdigheter lar CodePlane oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
codeplane søker etter disse stedene:

- Prosjektkonfigurasjon: `.codeplane/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/codeplane/skills/<name>/SKILL.md`
...
For prosjektlokale stier går codeplane opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codeplane/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.codeplane/skills/git-release/SKILL.md` slik:
...
codeplane viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `codeplane.json`:
...
**For innebygde agenter** (i `codeplane.json`):
