...
Custom commands let you specify a prompt you want to run from the web app or CLI.

```bash frame="none"
/my-command
```

Custom commands are in addition to the built-in commands like `/init`, `/undo`, `/redo`, `/share`, and `/help`.
...
Create `.codeplane/commands/test.md`:

```md title=".codeplane/commands/test.md"
---
...
You can add custom commands through the CodePlane config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your CodePlane [config](/docs/config):

```json title="codeplane.jsonc" {4-12}
{
  ...
  "command": {
    ...
    "test": {
      ...
      // This is shown as the description in command pickers
      "description": "Run tests with coverage",
      "agent": "build",
      "model": "anthropic/claude-3-5-sonnet-20241022"
    }
...
Now you can run this command from the web app:
...
- Global: `~/.config/codeplane/commands/`
- Per-project: `.codeplane/commands/`

```markdown title="~/.config/codeplane/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".codeplane/commands/component.md"
---
...
```md title=".codeplane/commands/create-file.md"
---
...
Use _!`command`_ to inject shell command output into your prompt.

For example, to create a custom command that analyzes test coverage:

```md title=".codeplane/commands/analyze-coverage.md"
---
...
```md title=".codeplane/commands/review-changes.md"
---
...
```md title=".codeplane/commands/review-component.md"
---
...
This is shown as the description when you type in the command.
...
codeplane includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, and `/help`.
