...
description: Skriv dine egne programtillegg for å utvide CodePlane.
---

Plugins lar deg utvide CodePlane ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CodePlane.
...
- `.codeplane/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/codeplane/node_modules/`.
...
1. Global konfigurasjon (`~/.config/codeplane/codeplane.json`)
2. Prosjektkonfigurasjon (`codeplane.json`)
3. Global plugin-katalog (`~/.config/codeplane/plugins/`)
4. Prosjektpluginkatalog (`.codeplane/plugins/`)
...
CodePlane kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CodePlane SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CodePlane.
...
:::note
Hvis du bruker CodePlane-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CodePlane fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CodePlane:
...
`tool`-hjelperen lager et tilpasset verktøy som CodePlane kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CodePlane sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://github.com/devinoldenburg/codeplane) for detaljer.
