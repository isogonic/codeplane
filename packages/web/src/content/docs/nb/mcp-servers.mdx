...
Du kan legge til eksterne verktøy i CodePlane ved å bruke _Model Context Protocol_ eller MCP. CodePlane støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CodePlane Config](https://github.com/devinoldenburg/codeplane) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codeplane.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/codeplane`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="codeplane.jsonc" {15}
{
  "$schema": "https://example.invalid/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CodePlane håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CodePlane:
...
Hvis serveren krever autentisering, vil CodePlane be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codeplane mcp auth <server-name>`.
...
```json title="codeplane.json" {7-11}
{
  "$schema": "https://example.invalid/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CodePlane lagre tokens sikkert i `~/.local/share/codeplane/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i CodePlane, sammen med innebygde verktøy. Så du kan administrere dem gjennom CodePlane-konfigurasjonen som alle andre verktøy.
...
```json title="codeplane.json" {11, 14-18}
{
  "$schema": "https://example.invalid/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CodePlane til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CodePlane-samtale oppført som en separat økt.
