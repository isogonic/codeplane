...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i CodePlane-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i CodePlane lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom CodePlane-konfigurasjonen.
...
CodePlane kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.codeplane/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `codeplane.json` konfigurasjonsfil:
...
- Globalt: `~/.config/codeplane/modes/`
- Prosjekt: `.codeplane/modes/`
...
Hvis ingen temperatur er spesifisert, bruker CodePlane modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CodePlane-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.codeplane/modes/` for prosjektspesifikke moduser eller `~/.config/codeplane/modes/` for globale moduser:
