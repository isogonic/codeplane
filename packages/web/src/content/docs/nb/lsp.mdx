---
title: LSP-servere
description: codeplane integreres med LSP-serverne dine.
---

CodePlane integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

CodePlane kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `CODEPLANE_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når codeplane åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i CodePlane-konfigurasjonen.
