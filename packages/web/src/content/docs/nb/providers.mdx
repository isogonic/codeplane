---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

CodePlane bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CodePlane-konfigurasjonen.
...
i `~/.local/share/codeplane/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i CodePlane
konfig.
...
CodePlane Zen er en liste over modeller levert av CodePlane-teamet som har vært
testet og verifisert for å fungere godt med CodePlane. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med CodePlane Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `CodePlane Zen` og gå til [example.invalid/auth](https://example.invalid/zen).
...
Det fungerer som alle andre leverandører i CodePlane og er helt valgfritt å bruke.

---

## CodePlane Go

CodePlane Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av CodePlane-teamet som har vært
testet og verifisert for å fungere godt med CodePlane.

1. Kjør kommandoen `/connect` i TUI, velg `CodePlane Go`, og gå til [example.invalid/auth](https://example.invalid/zen).
...
Slik bruker du Amazon Bedrock med CodePlane:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CodePlane:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codeplane.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codeplane støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CodePlane skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CodePlane-konfigurasjonen.
...
for å lagre token i CodePlane auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CodePlane bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CodePlane
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codeplane.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `codeplane.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CodePlane:
...
Slik bruker du Google Vertex AI med CodePlane:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CodePlane.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CodePlane, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) plugin for automatisk å logge hver CodePlane-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CodePlane-samtale oppført som en separat økt.
...
Du kan konfigurere CodePlane for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CodePlane for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CodePlane for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CodePlane. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/codeplane) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CodePlane:
...
3. Kopier API-nøkkelen for bruk i CodePlane.
...
6. **Viktig**: Før du bruker skymodeller i CodePlane, må du hente modellinformasjonen lokalt:
...
CodePlane Zen er en liste over testede og verifiserte modeller levert av CodePlane-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>CodePlane Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Skriv inn CodePlane API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CodePlane-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CodePlane-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodePlane:
...
Du kan også tilpasse modeller gjennom CodePlane-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CodePlane. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codeplane.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CodePlane forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codeplane auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CodePlane-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CodePlane-konfigurasjonen.
