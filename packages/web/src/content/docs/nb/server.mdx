---
title: Tjener
description: Samhandle med CodePlane-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codeplane serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CodePlane-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codeplane.local` |
...
Sett `CODEPLANE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codeplane` som standard, eller sett `CODEPLANE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codeplane serve` og `codeplane web`.
...
Når du kjører `codeplane` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CodePlane-serveren til å samhandle med CodePlane programmatisk.
:::

Denne arkitekturen lar CodePlane støtte flere klienter og lar deg samhandle med CodePlane programmatisk.

Du kan kjøre `codeplane serve` for å starte en frittstående server. Hvis du har
CodePlane TUI kjører, vil `codeplane serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CodePlane [IDE](/docs/ide) plugins.
...
codeplane-serveren viser følgende APIer.
