...
Det er to typer agenter i CodePlane; primære agenter og underagenter.
...
CodePlane kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CodePlane kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

CodePlane kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `codeplane.json`:
...
- Globalt: `~/.config/codeplane/agents/`
- Per prosjekt: `.codeplane/agents/`
...
Hvis ingen temperatur er spesifisert, bruker CodePlane modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CodePlane-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CodePlane-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [CodePlane Zen](/docs/zen), vil du bruke `codeplane/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `codeplane models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/devinoldenburg/codeplane).
:::
