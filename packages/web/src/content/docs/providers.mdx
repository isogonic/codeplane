---
title: Providers
description: Using any LLM provider in CodePlane.
---

import config from "../../../config.mjs"
export const console = config.console

CodePlane uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider from Settings -> Providers in the web app.
2. Configure the provider in your CodePlane config.

---

### Credentials

When you add a provider's API keys from Settings -> Providers, they are stored
...
You can customize the providers through the `provider` section in your CodePlane
config.
...
CodePlane Zen is a list of models provided by the CodePlane team that have been
tested and verified to work well with CodePlane. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CodePlane Zen.
:::

1. Open Settings -> Providers in the web app, select `CodePlane Zen`, and head to [example.invalid/auth](https://example.invalid/zen).
...
4. Open the model selector to see the list of models we recommend.

   ```txt
   /models
   ```

It works like any other provider in CodePlane and is completely optional to use.

---

## CodePlane Go

CodePlane Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CodePlane team that have been
tested and verified to work well with CodePlane.

1. Open Settings -> Providers in the web app, select `CodePlane Go`, and head to [example.invalid/auth](https://example.invalid/zen).
...
To use Amazon Bedrock with CodePlane:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running codeplane:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `codeplane.json`:
...
CodePlane. Anthropic explicitly prohibits this.

Previous versions of CodePlane came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CodePlane with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for codeplane to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your codeplane config.
...
GitLab Duo support in CodePlane is experimental. Features, configuration, and
behavior may change in future releases.
:::

CodePlane integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
CodePlane uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CodePlane
to only use your own GitLab-hosted instance, add the following to your
`codeplane.json` file. It is also recommended to disable session sharing.
...
Customize through `codeplane.json`:
...
When a `duo-workflow-*` model is selected, CodePlane will:
...
4. Route tool execution requests through CodePlane's permission-gated tool system
...
To use your GitHub Copilot subscription with codeplane:
...
To use Google Vertex AI with CodePlane:
...
2. Set the required environment variables:
   ...
   Set them while running codeplane.
...
In the event you see a feature or model from Helicone that isn't configured automatically through codeplane, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) plugin to automatically log each CodePlane conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CodePlane conversation listed as a separate session.
...
You can configure codeplane to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure codeplane to use local models through LM Studio.
...
You can configure codeplane to use local models through Ollama.

:::tip
Ollama can automatically configure itself for CodePlane. See the [Ollama integration docs](https://docs.ollama.com/integrations/codeplane) for details.
:::
...
To use Ollama Cloud with CodePlane:
...
3. Copy the API key for use in CodePlane.
...
6. **Important**: Before using cloud models in CodePlane, you must pull the model information locally:
...
CodePlane Zen is a list of tested and verified models provided by the CodePlane team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CodePlane Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Enter your CodePlane API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your codeplane config.
...
5. You can also customize them through your codeplane config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Codeplane:
...
You can also customize models through your codeplane config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with codeplane. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `codeplane.json` file in your project directory:
...
The `limit` fields allow CodePlane to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `codeplane auth list` to see if the credentials
...
2. For custom providers, check the codeplane config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your codeplane config.
