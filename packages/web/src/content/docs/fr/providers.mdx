---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

CodePlane utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration CodePlane.
...
en `~/.local/share/codeplane/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre CodePlane
configuration.
...
CodePlane Zen est une liste de modèles fournis par l'équipe CodePlane qui ont été
testé et vérifié pour fonctionner correctement avec CodePlane. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par CodePlane Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez codeplane et dirigez-vous vers [example.invalid/auth](https://example.invalid/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans CodePlane et son utilisation est totalement facultative.

---

## CodePlane Go

CodePlane Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe CodePlane qui ont été
testé et vérifié pour fonctionner correctement avec CodePlane.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `CodePlane Go` et rendez-vous sur [example.invalid/auth](https://example.invalid/zen).
...
Pour utiliser Amazon Bedrock avec CodePlane :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de codeplane :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY codeplane

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile codeplane

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX codeplane
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `codeplane.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans CodePlane n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que codeplane fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX codeplane
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX codeplane
   ```
...
Vous pouvez également ajouter des modèles via votre configuration codeplane.
...
pour stocker le jeton dans le stockage d'authentification codeplane.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
CodePlane utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller CodePlane
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `codeplane.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `codeplane.json` :
...
{
  "$schema": "https://example.invalid/config.json",
  "plugin": ["codeplane-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec codeplane :
...
Pour utiliser Google Vertex AI avec CodePlane :
...
Définissez-les lors de l'exécution de codeplane.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id codeplane
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via codeplane, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/codeplane/codeplane.jsonc"
{
  "$schema": "https://example.invalid/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) pour enregistrer automatiquement chaque conversation CodePlane en tant que session dans Helicone.

```bash
npm install -g codeplane-helicone-session
```
...
```json title="codeplane.json"
{
  "plugin": ["codeplane-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation CodePlane répertoriée comme une session distincte.
...
Vous pouvez configurer codeplane pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="codeplane.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://example.invalid/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer codeplane pour utiliser des modèles locaux via LM Studio.

```json title="codeplane.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://example.invalid/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer codeplane pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour CodePlane. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/codeplane) pour plus de détails.
:::

```json title="codeplane.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://example.invalid/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec CodePlane :
...
3. Copiez la clé API à utiliser dans CodePlane.
...
6. **Important** : Avant d'utiliser des modèles cloud dans CodePlane, vous devez extraire les informations du modèle localement :
...
CodePlane Zen est une liste de modèles testés et vérifiés fournie par l'équipe CodePlane. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>CodePlane Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé CodePlane API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration codeplane.
...
5. Vous pouvez également les personnaliser via votre configuration codeplane. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' codeplane
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group codeplane
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Codeplane :
...
Vous pouvez également personnaliser les modèles via votre configuration codeplane. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec codeplane. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in codeplane.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `codeplane.json` dans le répertoire de votre projet :

   ```json title="codeplane.json" ""myprovider"" {5-15}
   {
     "$schema": "https://example.invalid/config.json",
     "provider": {
       "myprovider": {
...
```json title="codeplane.json" {9,11,17-20}
{
  "$schema": "https://example.invalid/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à CodePlane de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `codeplane auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration codeplane et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration codeplane.
