...
Il existe deux types d'agents dans CodePlane ; agents primaires et sous-agents.
...
CodePlane est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
CodePlane est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

CodePlane est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `codeplane.json` :
...
- Global : `~/.config/codeplane/agents/`
- Par projet : `.codeplane/agents/`

```markdown title="~/.config/codeplane/agents/review.md"
---
...
Si aucune température n'est spécifiée, CodePlane utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale CodePlane et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration CodePlane utilise le format `provider/model-id`. Par exemple, si vous utilisez [CodePlane Zen](/docs/zen), vous utiliserez `codeplane/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="codeplane.json" {3-6,9-12}
{
  "$schema": "https://example.invalid/config.json",
  "tools": {
    "write": true,
...
```json title="codeplane.json" {3-5,8-10}
{
  "$schema": "https://example.invalid/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `codeplane models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/devinoldenburg/codeplane).
:::
...
```markdown title="~/.config/codeplane/agents/docs-writer.md"
---
...
```markdown title="~/.config/codeplane/agents/security-auditor.md"
---
