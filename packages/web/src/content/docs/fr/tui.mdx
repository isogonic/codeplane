---
title: TUI
description: Utilisation de l’interface utilisateur du terminal CodePlane.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodePlane fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de CodePlane démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le CodePlane TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à CodePlane. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez CodePlane. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `codeplane.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `CODEPLANE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
