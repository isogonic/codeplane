...
Avec CodePlane, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CodePlane utilise notre propre thème `codeplane`.
...
CodePlane est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CodePlane corresponde à l'apparence de leur terminal
...
CodePlane prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/codeplane/themes/*.json` ou `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codeplane/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codeplane/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/codeplane/themes
vim ~/.config/codeplane/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .codeplane/themes
vim .codeplane/themes/my-theme.json
```
...
{
  "$schema": "https://codeplane.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
