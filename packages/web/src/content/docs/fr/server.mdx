---
title: Serveur
description: Interagissez avec le serveur codeplane via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codeplane serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codeplane peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codeplane.local` |
...
Définissez `CODEPLANE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codeplane`, ou définissez `CODEPLANE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codeplane serve` et à `codeplane web`.
...
Lorsque vous exécutez `codeplane`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codeplane pour interagir avec codeplane par programmation.
:::

Cette architecture permet à codeplane de prendre en charge plusieurs clients et vous permet d'interagir avec codeplane par programme.

Vous pouvez exécuter `codeplane serve` pour démarrer un serveur autonome. Si vous avez le
codeplane TUI en cours d'exécution, `codeplane serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CodePlane [IDE](/docs/ide).
...
Le serveur codeplane expose les API suivantes.
