---
title: Server
description: Interact with codeplane server over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codeplane serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codeplane client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codeplane.local` |
...
Set `CODEPLANE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codeplane`, or set `CODEPLANE_SERVER_USERNAME` to override it. This applies to both `codeplane serve` and `codeplane web`.
...
When you run `codeplane` it starts the web app and a local server. The web app
talks to that server, which exposes an OpenAPI 3.1 spec endpoint. This endpoint
is also used to generate an [SDK](/docs/sdk).

:::tip
Use the codeplane server to interact with codeplane programmatically.
:::

This architecture lets codeplane support multiple clients and allows you to interact with codeplane programmatically.

You can run `codeplane serve` to start a standalone server without opening the
web app.
...
When you start the web app it randomly assigns a port and hostname. You can instead pass in the `--hostname` and `--port` [flags](/docs/cli), then connect browser clients or SDK clients to that server.
...
The codeplane server exposes the following APIs.
