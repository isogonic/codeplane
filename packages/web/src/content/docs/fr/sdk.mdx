---
title: SDK
description: Client JS de type sécurisé pour le serveur codeplane.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK codeplane JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler codeplane par programme.
...
Créez une instance de codeplane :

```javascript
import { createCodeplane } from "@codeplane-ai/sdk"

const { client } = await createCodeplane()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `codeplane.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${codeplane.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de codeplane, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@codeplane-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*codeplane" },
})
