---
title: Plugins
description: Écrivez vos propres plugins pour étendre CodePlane.
---

Les plugins vous permettent d'étendre CodePlane en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CodePlane.
...
- `.codeplane/plugins/` - Plugins au niveau du projet
- `~/.config/codeplane/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/codeplane/node_modules/`.
...
1. Configuration globale (`~/.config/codeplane/codeplane.json`)
2. Configuration du projet (`codeplane.json`)
3. Répertoire global des plugins (`~/.config/codeplane/plugins/`)
4. Répertoire des plugins du projet (`.codeplane/plugins/`)
...
CodePlane exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codeplane pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre codeplane.
...
:::note
Si vous utilisez l'application de bureau CodePlane, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codeplane de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à codeplane :
...
L'assistant `tool` crée un outil personnalisé que codeplane peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codeplane aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://github.com/devinoldenburg/codeplane) pour plus de détails.
