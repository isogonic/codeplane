...
Most popular providers are preloaded by default. If you've added provider credentials from Settings -> Providers, they'll be available when you start Codeplane.
...
Once you've configured your provider you can select the model you want from the model selector in the web app.
...
Here are several models that work well with Codeplane, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Codeplane Zen](/docs/zen), you would use `codeplane/gpt-5.1-codex` for GPT 5.1 Codex.
...
Many models support multiple variants with different configurations. Codeplane ships with built-in default variants for popular providers.

### Built-in variants

Codeplane ships with default variants for many providers:
...
Use the model selector to switch between variants.

---

## Loading models

When Codeplane starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Codeplane config.
