...
Vous pouvez ajouter des outils externes à CodePlane à l'aide du _Model Context Protocol_ ou MCP. CodePlane prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [CodePlane Config](https://github.com/devinoldenburg/codeplane) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/codeplane`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
CodePlane gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, CodePlane :
...
Si le serveur nécessite une authentification, CodePlane vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `codeplane mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, CodePlane stockera les jetons en toute sécurité dans `~/.local/share/codeplane/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans CodePlane, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration CodePlane comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter CodePlane à votre compte Sentry.
