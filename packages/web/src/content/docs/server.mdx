...
When you run `codeplane` it starts a TUI and a server. Where the TUI is the
...
You can run `codeplane serve` to start a standalone server. If you have the
codeplane TUI running, `codeplane serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CodePlane [IDE](/docs/ide) plugins.
