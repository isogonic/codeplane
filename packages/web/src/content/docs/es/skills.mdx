...
Las habilidades del agente permiten a Codeplane descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Codeplane busca estas ubicaciones:

- Configuración del proyecto: `.codeplane/skills/<name>/SKILL.md`
- Configuración global: `~/.config/codeplane/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Codeplane sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codeplane/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/codeplane/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.codeplane/skills/git-release/SKILL.md` así:
...
Codeplane enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codeplane.json`:
...
**Para agentes integrados** (en `codeplane.json`):
