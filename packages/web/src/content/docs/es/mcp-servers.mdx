...
You can add external tools to Codeplane using the _Model Context Protocol_, or MCP. Codeplane supports both local and remote servers.
...
You can define MCP servers in your [Codeplane Config](https://github.com/devinoldenburg/codeplane) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/codeplane` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Codeplane automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Codeplane will:
...
If the server requires authentication, Codeplane will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `codeplane mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Codeplane will store the tokens securely in `~/.local/share/codeplane/mcp-auth.json`.
...
Your MCPs are available as tools in Codeplane, alongside built-in tools. So you can manage them through the Codeplane config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Codeplane to your Sentry account.
