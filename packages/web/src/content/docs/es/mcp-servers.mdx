...
Puede agregar herramientas externas a CodePlane utilizando el _Model Context Protocol_ o MCP. CodePlane admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CodePlane Config](https://github.com/devinoldenburg/codeplane) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/codeplane`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
CodePlane maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CodePlane:
...
Si el servidor requiere autenticación, CodePlane le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `codeplane mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CodePlane almacenará los tokens de forma segura en `~/.local/share/codeplane/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CodePlane, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CodePlane como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CodePlane a su cuenta Sentry.
