...
Las herramientas permiten que LLM realice acciones en su código base. CodePlane viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en CodePlane.
...
:::note
Esta herramienta solo está disponible cuando `CODEPLANE_EXPERIMENTAL_LSP_TOOL=true` (o `CODEPLANE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor CodePlane o cuando la variable de entorno `CODEPLANE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar CodePlane:

```bash
CODEPLANE_ENABLE_EXA=1 codeplane
```
