---
title: Servidor
description: Interactuar con el servidor codeplane a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codeplane serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codeplane puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codeplane.local` |
...
Configure `CODEPLANE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codeplane`, o configure `CODEPLANE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codeplane serve` como a `codeplane web`.
...
Cuando ejecuta `codeplane`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codeplane para interactuar con codeplane mediante programación.
:::

Esta arquitectura permite que codeplane admita múltiples clientes y le permite interactuar con codeplane mediante programación.

Puede ejecutar `codeplane serve` para iniciar un servidor independiente. Si tienes el
codeplane TUI ejecutándose, `codeplane serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CodePlane [IDE](/docs/ide).
...
El servidor codeplane expone las siguientes API.
