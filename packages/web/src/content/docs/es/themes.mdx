...
Con CodePlane puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, CodePlane usa nuestro propio tema `codeplane`.
...
CodePlane viene con varios temas integrados.
...
- Quiere que CodePlane coincida con la apariencia de su terminal
...
CodePlane admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/codeplane/themes/*.json` o `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.codeplane/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.codeplane/themes/*.json`
