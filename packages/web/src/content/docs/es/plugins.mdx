---
title: Complementos
description: Escriba sus propios complementos para extender CodePlane.
---

Los complementos le permiten extender CodePlane al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CodePlane.
...
- `.codeplane/plugins/` - Complementos a nivel de proyecto
- `~/.config/codeplane/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/codeplane/node_modules/`.
...
1. Configuración global (`~/.config/codeplane/codeplane.json`)
2. Configuración del proyecto (`codeplane.json`)
3. Directorio global de complementos (`~/.config/codeplane/plugins/`)
4. Directorio de complementos del proyecto (`.codeplane/plugins/`)
...
CodePlane ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK codeplane para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar codeplane.
...
:::note
Si está utilizando la aplicación de escritorio CodePlane, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codeplane lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a codeplane:
...
El ayudante `tool` crea una herramienta personalizada a la que codeplane puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codeplane junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://github.com/devinoldenburg/codeplane) para obtener más detalles.
