...
description: Cree herramientas que LLM pueda llamar en codeplane.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de codeplane como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.codeplane/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/codeplane/tools/`.
...
```ts title=".codeplane/tools/database.ts" {1}
...
```ts title=".codeplane/tools/math.ts"
import { tool } from "@codeplane-ai/plugin"
...
```ts title=".codeplane/tools/bash.ts"
import { tool } from "@codeplane-ai/plugin"
...
```ts title=".codeplane/tools/project.ts" {8}
import { tool } from "@codeplane-ai/plugin"
...
```python title=".codeplane/tools/add.py"
import sys
...
```ts title=".codeplane/tools/python-add.ts" {10}
import { tool } from "@codeplane-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codeplane/tools/add.py")
