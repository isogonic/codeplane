---
title: Proveedores
description: Usando cualquier proveedor LLM en CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

CodePlane usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración CodePlane.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu CodePlane
configuración.
...
CodePlane Zen es una lista de modelos proporcionados por el equipo CodePlane que han sido
probado y verificado para funcionar bien con CodePlane. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con CodePlane Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione codeplane y diríjase a [example.invalid/auth](https://example.invalid/auth).
...
Funciona como cualquier otro proveedor en CodePlane y su uso es completamente opcional.

---

## CodePlane Go

CodePlane Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de CodePlane que han sido
probado y verificado para funcionar bien con CodePlane.

1. Ejecute el comando `/connect` en TUI, seleccione `CodePlane Go` y diríjase a [example.invalid/auth](https://example.invalid/zen).
...
Para usar Amazon Bedrock con CodePlane:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta codeplane:
   ...
   Para una configuración persistente o específica del proyecto, utilice `codeplane.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en CodePlane no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que codeplane funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración codeplane.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `CodePlane`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación codeplane.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
CodePlane utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear CodePlane
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `codeplane.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `codeplane.json`:
...
Para utilizar su suscripción GitHub Copilot con codeplane:
...
Para utilizar Google Vertex AI con CodePlane:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta codeplane.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de codeplane, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) para registrar automáticamente cada conversación CodePlane como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación CodePlane enumerada como una sesión separada.
...
Puede configurar codeplane para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar codeplane para usar modelos locales a través de LM Studio.
...
Puedes configurar codeplane para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para CodePlane. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/codeplane) para obtener más detalles.
:::
...
Para usar Ollama Cloud con CodePlane:
...
3. Copie la clave API para usarla en CodePlane.
...
6. **Importante**: Antes de usar modelos en la nube en CodePlane, debe obtener la información del modelo localmente:
...
CodePlane Zen es una lista de modelos probados y verificados proporcionada por el equipo CodePlane. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>CodePlane Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave CodePlane API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración codeplane.
...
5. También puedes personalizarlos a través de tu configuración codeplane. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con CodePlane:
...
También puede personalizar modelos a través de su configuración codeplane. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con codeplane. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `codeplane.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a CodePlane comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `codeplane auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración codeplane y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración codeplane.
