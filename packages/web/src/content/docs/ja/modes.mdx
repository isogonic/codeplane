...
:::caution
モードは、Codeplane 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、Codeplane 設定を通じて独自の設定を行います。
...
Codeplane には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.codeplane/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`codeplane.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/codeplane/modes/`
- プロジェクト: `.codeplane/modes/`
...
温度が指定されていない場合、Codeplane はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.codeplane/modes/` に、グローバルモードの場合は `~/.config/codeplane/modes/` に作成します。
