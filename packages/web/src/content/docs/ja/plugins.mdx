---
title: プラグイン
description: CodePlane を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで CodePlane を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、CodePlane のデフォルトの動作を変更したりできます。
...
- `.codeplane/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/codeplane/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/codeplane/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/codeplane/codeplane.json`)
2. プロジェクト設定 (`codeplane.json`)
3. グローバルプラグインディレクトリ (`~/.config/codeplane/plugins/`)
4. プロジェクトプラグインディレクトリ (`.codeplane/plugins/`)
...
CodePlane は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための CodePlane SDK クライアント。
...
ここでは、CodePlane を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://github.com/devinoldenburg/codeplane) を参照してください。
...
CodePlane が `.env` ファイルを読み取らないようにします。
...
プラグインは CodePlane にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、CodePlane が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して CodePlane で使用できます。
