---
title: サーバー
description: HTTP 経由で CodePlane サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codeplane serve` コマンドは、CodePlane クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `codeplane.local` |
...
HTTP 基本認証でサーバーを保護するには、`CODEPLANE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `codeplane` になるか、`CODEPLANE_SERVER_USERNAME` を設定してオーバーライドします。これは、`codeplane serve` と `codeplane web` の両方に当てはまります。
...
`codeplane` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
CodePlane サーバーを使用して、プログラムで CodePlane と対話します。
:::

`codeplane serve` を実行してスタンドアロンサーバーを起動できます。すでに CodePlane TUI を実行している場合でも、`codeplane serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、CodePlane [IDE](/docs/ide) プラグインによって使用されます。
