...
Med CodePlane kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CodePlane vårt eget `codeplane`-tema.
...
CodePlane støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/codeplane/themes/*.json` eller `$XDG_CONFIG_HOME/codeplane/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.codeplane/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.codeplane/themes/*.json`
