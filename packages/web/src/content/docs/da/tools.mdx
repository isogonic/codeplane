...
Her er alle de innebygde verktøyene som er tilgængelige i CodePlane.
...
:::note
Dette verktøyet er bare tilgængelig når `CODEPLANE_EXPERIMENTAL_LSP_TOOL=true` (eller `CODEPLANE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger CodePlane-leverandøren eller når miljøvariabelen `CODEPLANE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
