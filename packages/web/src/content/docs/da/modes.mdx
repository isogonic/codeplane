...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i codeplane-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i codeplane giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem codeplane-konfigurationen.
...
codeplane leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.codeplane/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `codeplane.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger codeplane modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale codeplane-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.codeplane/modes/` for projektspecifikke tilstande eller `~/.config/codeplane/modes/` for globale tilstande:
