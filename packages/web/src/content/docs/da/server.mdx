---
title: Server
description: Interager med codeplane-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codeplane serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en codeplane-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `codeplane.local` |
...
Indstil `CODEPLANE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `codeplane`, eller indstil `CODEPLANE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `codeplane serve` og `codeplane web`.
...
Når du kører `codeplane` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug codeplane-serveren til at interagere med codeplane programmatisk.
:::

Denne arkitektur lader codeplane understøtte flere klienter og lader dig interagere med codeplane programmatisk.

Du kan køre `codeplane serve` for at starte en selvstændig server. Hvis du har
codeplane TUI kørende, vil `codeplane serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CodePlane [IDE](/docs/ide) plugins.
...
CodePlane-serveren viser følgende API'er.
