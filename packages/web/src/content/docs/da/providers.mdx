---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CodePlane.
---

import config from "../../../../config.mjs"
export const console = config.console

CodePlane bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CodePlane-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/codeplane/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i CodePlane-konfigurationen.
...
CodePlane Zen er en liste over modeller leveret af CodePlane-teamet, som er blevet testet og verificeret til at fungere godt med CodePlane. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med CodePlane Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codeplane og gå til [example.invalid/auth](https://example.invalid/auth).
...
Det fungerer som alle andre udbydere i CodePlane og er helt valgfrit at bruge.

---

## CodePlane Go

CodePlane Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CodePlane-teamet, som er testet og verificeret til at fungere godt med CodePlane.

1. Kør kommandoen `/connect` i TUI, vælg `CodePlane Go`, og gå til [example.invalid/auth](https://example.invalid/zen).
...
Sådan bruger du Amazon Bedrock med CodePlane:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører codeplane:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codeplane.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CodePlane understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codeplane skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem codeplane-konfigurationen.
...
gemme tokenet i codeplane auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CodePlane bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CodePlane
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codeplane.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med CodePlane:
...
Sådan bruger du Google Vertex AI med CodePlane:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører codeplane.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem codeplane, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [codeplane-helicone-session](https://github.com/H2Shami/codeplane-helicone-session) plugin for automatisk at logge hver CodePlane-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CodePlane-samtale opført som en separat session.
...
Du kan konfigurere codeplane til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codeplane til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codeplane til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CodePlane. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/codeplane) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CodePlane:
...
3. Kopier API-nøglen til brug i CodePlane.
...
6. **Vigtigt**: Før du bruger skymodeller i CodePlane, skal du hente modelinformationen lokalt:
...
CodePlane Zen er en liste over testede og verificerede modeller leveret af CodePlane-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>CodePlane Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **CodePlane Zen**.

   ```txt
   /connect
   ```

3. Indtast CodePlane API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem codeplane-konfigurationen.
...
5. Du kan også tilpasse dem gennem codeplane-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodePlane:
...
Du kan også tilpasse modeller gennem codeplane-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med codeplane. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `codeplane.json`-filen i projektkataloget:
...
`limit`-felterne lader CodePlane forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codeplane auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek codeplane-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i codeplane-konfigurationen.
