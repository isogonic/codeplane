...
Der er to typer agenter i CodePlane; primære agenter og subagenter.
...
CodePlane leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
CodePlane leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

CodePlane leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `codeplane.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger CodePlane modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale CodePlane-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din CodePlane-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [CodePlane Zen](/docs/zen), vil du bruge `codeplane/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `codeplane models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/devinoldenburg/codeplane).
:::
