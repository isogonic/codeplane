...
There are two types of agents in CodePlane; primary agents and subagents.
...
CodePlane comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
CodePlane comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

CodePlane comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `codeplane.json` config file:
...
If no temperature is specified, CodePlane uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global CodePlane config and the project specific config.
...
The model ID in your CodePlane config uses the format `provider/model-id`. For example, if you're using [CodePlane Zen](/docs/zen), you would use `codeplane/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `codeplane models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/devinoldenburg/codeplane).
:::
