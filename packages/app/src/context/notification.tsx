...
export const { use: useNotification, provider: NotificationProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      unseenCount() {
        return visibleList().reduce((count, notification) => count + (notification.viewed ? 0 : 1), 0)
      },
      unseenHasError() {
        return visibleList().some((notification) => !notification.viewed && notification.type === "error")
      },
      markAllViewed() {
