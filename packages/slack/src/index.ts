...
import { createCodeplane, type ToolPart } from "@codeplane-ai/sdk"
...
console.log("🚀 Starting codeplane server...")
const codeplane = await createCodeplane({
  port: 0,
})
console.log("✅ Codeplane server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await codeplane.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    ...
    console.log("✅ Created codeplane session:", createResult.data.id)
