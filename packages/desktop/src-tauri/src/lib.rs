...
async fn initialize(app: AppHandle) {
    ...
    let _ = ready_tx.send(ServerReadyData {
        url: url.clone(),
        username: Some("codeplane".to_string()),
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %codeplane_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("CODEPLANE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("CODEPLANE_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = codeplane_db_path() else {
        return true;
    };
...
fn codeplane_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("codeplane").join("codeplane.db"))
}
