...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("codeplane-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("codeplane_lib=debug,codeplane_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("codeplane_lib=info,codeplane_desktop=info,sidecar=info")
        }
    });
