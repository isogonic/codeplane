...
const CLI_INSTALL_DIR: &str = ".codeplane/bin";
const CLI_BINARY_NAME: &str = "codeplane";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("codeplane-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "CODEPLANE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "CODEPLANE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("CODEPLANE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.codeplane/bin/codeplane\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://codeplane.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "CODEPLANE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "CODEPLANE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "CODEPLANE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "CODEPLANE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "CODEPLANE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "CODEPLANE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("CODEPLANE_SERVER_USERNAME", "codeplane".to_string()),
        ("CODEPLANE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn codeplane");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("CODEPLANE_CLIENT"), Some(&"desktop".to_string()));
    }
