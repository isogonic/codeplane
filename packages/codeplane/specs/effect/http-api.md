...
- `CODEPLANE_EXPERIMENTAL_HTTPAPI` gates the bridge. Default behavior still uses Hono.
...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-codeplane-directory`.
...
- Cover `directory` and `x-codeplane-directory` instance selection.
...
- [x] Bridge selected `HttpApi` routes into Hono behind `CODEPLANE_EXPERIMENTAL_HTTPAPI`.
