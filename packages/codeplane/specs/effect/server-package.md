...
Practical reference for extracting a future `packages/server` from the current `packages/codeplane` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all codeplane services, Effect-first source of truth
- `packages/server` - codeplane server, with separate contract and implementation, still producing `openapi.json`
- `packages/cli` - web-first CLI entrypoints
...
- import from `server` and embed the full codeplane server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/codeplane`.
...
- those slices are mounted into the Hono server behind `CODEPLANE_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `codeplane` package split is earlier in the migration.
...
- the current server host still lives in `packages/codeplane`
...
- if `packages/server` imports services from `packages/codeplane` while `packages/codeplane` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/codeplane` as the temporary Hono host
...
- `packages/server` must not import from `packages/codeplane`

Allowed in phase 1:

- `packages/codeplane` imports `packages/server`
...
- `packages/codeplane` shrinks or disappears as package responsibilities are fully split
...
- `packages/codeplane` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/codeplane`
...
Keep in `packages/codeplane` for now:
...
- host mounting from `packages/codeplane`
...
- no imports from `packages/codeplane` inside `packages/server`
...
- `packages/server` must still not import from `packages/codeplane`
...
### PR 4. Mount `packages/server` question from `packages/codeplane`

Scope:

- replace local experimental question route wiring in `packages/codeplane`
...
- mount it in parallel from `packages/codeplane`
...
- move adapter selection and server startup out of `packages/codeplane`

Rules:

- do not start this while `packages/server` still depends on `packages/codeplane`
...
- host logic no longer lives in `packages/codeplane`
...
3. the route is mounted from the current host in `packages/codeplane`
...
- `packages/server` imports services or runtime from `packages/codeplane`
- `packages/codeplane` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/codeplane` imports
...
- [ ] mount `question` from `packages/codeplane`
...
2. keep `packages/codeplane` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/codeplane`, stop and restructure the boundary first.
