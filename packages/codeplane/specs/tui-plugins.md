...
- Author package entrypoint is `@codeplane-ai/plugin/tui`.
...
- Import types from `@codeplane-ai/plugin/tui`.
- `@codeplane-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@codeplane-ai/plugin/tui"
...
- The value is a semver range checked against the running CodePlane version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codeplane` is absent, no check is performed (backward compatible).
...
- `codeplane plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `codeplane plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.codeplane` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codeplane`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codeplane`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codeplane` dir or `CODEPLANE_CONFIG_DIR`, CodePlane installs `@codeplane-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codeplane-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.codeplane/themes` area near the plugin config source.
...
- `--pure` / `CODEPLANE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codeplane/plugins/tui-smoke.tsx`
- Local vim plugin: `.codeplane/plugins/tui-vim.tsx`
- Local smoke config: `.codeplane/tui.json`
- Local smoke theme: `.codeplane/plugins/smoke-theme.json`
