...
Replace runtime Bun shell template-tag usage in `packages/codeplane/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/codeplane/src` is removed except:
