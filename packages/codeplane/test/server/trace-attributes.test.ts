...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under codeplane.", () => {
    expect(paramToAttributeKey("name")).toBe("codeplane.name")
    expect(paramToAttributeKey("slug")).toBe("codeplane.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["codeplane.sessionID"]).toBeUndefined()
    expect(attrs["codeplane.messageID"]).toBeUndefined()
    expect(attrs["codeplane.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under codeplane. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["codeplane.name"]).toBe("exa")
