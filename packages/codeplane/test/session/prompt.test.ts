...
const mcp = Layer.succeed(
  ...
  MCP.Service.of({
    ...
    beginAuth: () => Effect.die("unexpected MCP auth in prompt-effect tests"),
...
it.live(
  "failed task tool preserves metadata on error tool state",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        ...
        expect(tool.state.metadata?.sessionId).toBeDefined()
        expect(tool.state.metadata?.status).toBe("error")
        expect(tool.state.metadata?.agent).toBe("general")
        expect(tool.state.metadata?.model).toEqual({
          providerID: ProviderID.make("test"),
          modelID: ModelID.make("missing-model"),
        })
...
it.live(
  "runs consecutive non-command subtasks concurrently",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const chat = yield* sessions.create({ title: "Pinned" })
        yield* llm.push(
          reply().wait(gate.promise).text("first child").stop(),
          reply().wait(gate.promise).text("second child").stop(),
          reply().text("parent done").stop(),
        )
        const msg = yield* user(chat.id, "hello")
        yield* addSubtask(chat.id, msg.id, ref, { description: "first inspect" })
...
it.live(
  "runs consecutive command subtasks sequentially",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const gate = defer<void>()
        ...
        yield* llm.push(
          reply().wait(gate.promise).text("first child").stop(),
          reply().text("second child").stop(),
          reply().text("parent done").stop(),
        )
        const msg = yield* user(chat.id, "hello")
        yield* addSubtask(chat.id, msg.id, ref, { command: "first", description: "first inspect" })
        yield* addSubtask(chat.id, msg.id, ref, { command: "second", description: "second inspect" })

        const fiber = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
        yield* llm.wait(1)
        const beforeFirstCompletes = yield* llm.wait(2).pipe(Effect.timeout("300 millis"), Effect.exit)
        expect(Exit.isFailure(beforeFirstCompletes)).toBe(true)

        gate.resolve()
        yield* llm.wait(2).pipe(Effect.timeout("1 second"))
        const exit = yield* Fiber.await(fiber)
        expect(Exit.isSuccess(exit)).toBe(true)
        expect(yield* llm.calls).toBe(3)
        expect(yield* sessions.children(chat.id)).toHaveLength(2)
...
it.live("prompt tools mask does not overwrite session permissions", () =>
  provideTmpdirInstance(
    (_dir) =>
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
...
it.live("does not loop empty assistant turns for a simple reply", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      ...
      yield* llm.text("packages/codeplane/src/session/processor.ts")
    ...
    (_dir) =>
      Effect.gen(function* () {
        ...
        const other = yield* prompt.prompt({
          sessionID: session.id,
          agent: "build",
          model: { providerID: ProviderID.make("codeplane"), modelID: ModelID.make("kimi-k2.5-free") },
          noReply: true,
          parts: [{ type: "text", text: "hello" }],
        })
