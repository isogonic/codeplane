...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const codeplaneFile = path.join(tmp.path, ".git", "codeplane")
    expect(await Bun.file(codeplaneFile).exists()).toBe(false)
  })
  ...
  test("should handle git repository with commits", async () => {
    ...
    const codeplaneFile = path.join(tmp.path, ".git", "codeplane")
    expect(await Bun.file(codeplaneFile).exists()).toBe(true)
  })
  ...
  test("does not automatically discover project favicons", async () => {
    const original = Flag.CODEPLANE_EXPERIMENTAL_ICON_DISCOVERY
    Flag.CODEPLANE_EXPERIMENTAL_ICON_DISCOVERY = true
    try {
      await using tmp = await tmpdir({ git: true })
      ...
      Flag.CODEPLANE_EXPERIMENTAL_ICON_DISCOVERY = original
    }
  })
})
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "codeplane")
...
describe("Project.fromDirectory with bare repos", () => {
  test("worktree from bare repo should cache in bare repo, not parent", async () => {
    ...
    try {
      ...
      const correctCache = path.join(barePath, "codeplane")
      const wrongCache = path.join(parentDir, ".git", "codeplane")
  ...
  test("different bare repos under same parent should not share project ID", async () => {
    ...
    try {
      ...
      const cacheA = path.join(bareA, "codeplane")
      const cacheB = path.join(bareB, "codeplane")
      const wrongCache = path.join(parentDir, ".git", "codeplane")
