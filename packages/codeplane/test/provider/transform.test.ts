...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, codeplaneModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.codeplane?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.codeplane?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          codeplane: { itemId: "msg_codeplane" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              codeplane: { itemId: "msg_codeplane_part" },
    ...
    expect(result[0].providerOptions?.codeplane?.itemId).toBe("msg_codeplane")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.codeplane?.itemId).toBe("msg_codeplane_part")
