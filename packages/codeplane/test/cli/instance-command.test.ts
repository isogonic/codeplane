...
describe("cli instance helpers", () => {
  ...
  test("rejects basic auth fields with control characters", () => {
    expect(() => composeRemoteHeaders({ username: "ali\nce", password: "secret" })).toThrow(/control characters/)
  ...
  test("allows colons in basic auth passwords", () => {
    expect(
      composeRemoteHeaders({
        username: "alice",
        password: "pass:word",
      ...
      Authorization: "Basic YWxpY2U6cGFzczp3b3Jk",
    })
  })
  ...
  test("rejects unsafe explicit instance ids", () => {
    ...
    expect(() => validateInstanceID("..")).toThrow(/cannot be . or ../)
    expect(() => validateInstanceID(".local")).toThrow(/cannot start with a dot/)
  ...
  test("normalizes tagged saved local instance versions", () => {
    expect(
      localInstanceVersions({
        instances: [
  ...
  test("formats local status version for scripts", () => {
    expect(
      formatLocalStatus(
        {
          binaryVersion: "28.2.2",
  ...
  test("rejects conflicting local status script flags", () => {
    ...
    ).toThrow(/--path-only, --installed-only, or --version-only/)
    expect(() =>
      formatLocalStatus(
        {
          binaryVersion: "28.2.2",
  ...
  test("trims local status binary version in JSON and script output", () => {
    ...
    expect(JSON.parse(formatLocalStatus(status)).binaryVersion).toBe("28.2.2")
    expect(formatLocalStatus(status, false, false, true)).toBe("28.2.2")
  })
  ...
  test("formats local runtime versions with a limit", () => {
    expect(
      ...
      selectedPrereleaseVersionCount: 0,
  ...
  test("normalizes tagged local runtime latest JSON output", () => {
  ...
  test("formats ranged local runtime versions for scripts", () => {
    expect(
      formatLocalVersions(
        {
          distTags: {},
          versions: ["29.0.0", "28.3.0", "28.2.1", "27.9.9"],
        },
        10,
        undefined,
        undefined,
  ...
  test("formats the newest selected local runtime version for scripts", () => {
    expect(
      formatLocalVersions(
        {
          distTags: {},
  ...
  test("formats the latest ranged stable local runtime version for scripts", () => {
    expect(
      formatLocalVersions(
        {
          distTags: {},
          versions: ["29.0.0", "28.4.0-beta.1", "28.3.0", "28.2.1"],
        },
        1,
        undefined,
        undefined,
  ...
  test("formats the latest prerelease selected local runtime version for scripts", () => {
    expect(
      formatLocalVersions(
        {
          distTags: {},
          versions: ["29.0.0", "28.4.0-beta.1", "28.3.0-rc.1", "28.2.1"],
        },
        1,
        undefined,
        28,
  ...
  test("formats the latest ranged prerelease local runtime version for scripts", () => {
    expect(
      formatLocalVersions(
        {
          distTags: {},
          versions: ["29.0.0-beta.1", "28.4.0-beta.1", "28.3.0-rc.1", "28.2.1"],
        },
        1,
        undefined,
        undefined,
  ...
  test("rejects conflicting latest-prerelease-only local runtime flags", () => {
    expect(() =>
      formatLocalVersions(
        { distTags: {}, versions: [] },
        10,
  ...
  test("rejects conflicting newest-only local runtime flags", () => {
    expect(() =>
      formatLocalVersions({ distTags: {}, versions: [] }, 10, undefined, undefined, false, false, false, false, false, true, false, false, true),
    ).toThrow(/Use --newest-only/)
  })
})
