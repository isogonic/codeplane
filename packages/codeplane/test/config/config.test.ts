...
async function writeManagedSettings(settings: object, filename = "codeplane.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "codeplane.json") {
...
test("ignores legacy tui keys in codeplane config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://example.invalid/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "codeplane.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["CODEPLANE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { codeplane: { options: { apiKey: "{env:CODEPLANE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["codeplane"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["CODEPLANE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CODEPLANE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "codeplane.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .codeplane directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeplaneDir = path.join(dir, ".codeplane")
      await fs.mkdir(codeplaneDir, { recursive: true })
      const agentDir = path.join(codeplaneDir, "agent")
...
test("loads agents from .codeplane/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const agentsDir = path.join(codeplaneDir, "agents")
...
test("loads commands from .codeplane/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeplaneDir = path.join(dir, ".codeplane")
      await fs.mkdir(codeplaneDir, { recursive: true })

      const commandDir = path.join(codeplaneDir, "command")
...
test("loads commands from .codeplane/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codeplaneDir = path.join(dir, ".codeplane")
      await fs.mkdir(codeplaneDir, { recursive: true })

      const commandsDir = path.join(codeplaneDir, "commands")
...
test("does not try to install dependencies in read-only CODEPLANE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
...
test("installs dependencies in writable CODEPLANE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "codeplane.json"),
        JSON.stringify({ $schema: "https://example.invalid/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .codeplane config
      const projectDir = path.join(dir, "project")
      const codeplaneDir = path.join(projectDir, ".codeplane")
      await fs.mkdir(codeplaneDir, { recursive: true })
      ...
      // Local .codeplane config with different plugins
      await Filesystem.write(
        path.join(codeplaneDir, "codeplane.json"),
        JSON.stringify({
          $schema: "https://example.invalid/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codeplane config with some overlapping plugins
      await Filesystem.write(
        path.join(codeplaneDir, "codeplane.json"),
        JSON.stringify({
          $schema: "https://example.invalid/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".codeplane")
      ...
      await Filesystem.write(
        path.join(local, "codeplane.json"),
        JSON.stringify({
          $schema: "https://example.invalid/config.json",
...
// Note: preload.ts sets CODEPLANE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .codeplane config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codeplane directory config enables it
      const codeplaneDir = path.join(dir, ".codeplane")
      await fs.mkdir(codeplaneDir, { recursive: true })
      await Filesystem.write(
        path.join(codeplaneDir, "codeplane.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/codeplane")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/codeplane")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "codeplane.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-codeplane@2.4.3", file)).toBe("oh-my-codeplane@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-codeplane@2.4.3", "file:///project/.codeplane/plugin/oh-my-codeplane.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.codeplane/plugin/demo.ts", "file:///project/.codeplane/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.codeplane/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const codeplaneDir = path.join(projectDir, ".codeplane")
        const pluginDir = path.join(codeplaneDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("CODEPLANE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"]
    process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        ...
        process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .codeplane/ directories when flag is set", async () => {
    const originalEnv = process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"]
    process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .codeplane directory with a command
          const codeplaneDir = path.join(dir, ".codeplane", "command")
          await fs.mkdir(codeplaneDir, { recursive: true })
          await Filesystem.write(path.join(codeplaneDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .codeplane should NOT be in directories list
          const hasProjectCodeplane = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectCodeplane).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODEPLANE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CODEPLANE_CONFIG_DIR"]
      process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CODEPLANE_CONFIG_DIR"]
      } else {
        process.env["CODEPLANE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CODEPLANE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODEPLANE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CODEPLANE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CODEPLANE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from CODEPLANE_CONFIG_DIR, not project
...
describe("CODEPLANE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CODEPLANE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CODEPLANE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CODEPLANE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://example.invalid/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CODEPLANE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CODEPLANE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CODEPLANE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CODEPLANE_CONFIG_CONTENT"]
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info.zod,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://example.invalid/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://example.invalid/config.json")
})
