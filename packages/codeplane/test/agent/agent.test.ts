...
test("plan agent denies edits except .codeplane/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".codeplane/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".codeplane", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  const home = process.env.CODEPLANE_TEST_HOME
  process.env.CODEPLANE_TEST_HOME = tmp.path
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".codeplane", "skill", "perm-skill")
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // all visible primary agents are disabled, so only hidden/internal primaries remain
