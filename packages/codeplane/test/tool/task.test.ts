...
function stubOps(opts?: {
  ...
  reply?: (input: SessionPrompt.PromptInput) => MessageV2.WithParts
  text?: string
}): TaskPromptOps {
  return {
    cancel() {},
...
describe("tool.task", () => {
  ...
  it.live("execute rejects primary-only agents and suggests valid subagents", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const { chat, assistant } = yield* seed()
        const tool = yield* TaskTool
        ...
        if (Exit.isFailure(exit)) {
          ...
          if (error instanceof Error) {
            expect(error.message).toContain("Invalid subagent type: build")
  ...
  it.live("execute returns assistant errors when the child has no final text", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const { chat, assistant } = yield* seed()
        const tool = yield* TaskTool
        ...
        const promptOps = stubOps({
          reply: (input) => ({
            info: {
              ...
              providerID: input.model?.providerID ?? ref.providerID,
              time: { created: Date.now() },
              error: new MessageV2.ContextOverflowError({ message: "context exploded" }).toObject(),
            },
            parts: [],
          }),
        })
        ...
        expect(result.output).toContain("ContextOverflowError: context exploded")
      }),
    ),
  )

  it.live("execute returns final tool errors when the child has no final text", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const { chat, assistant } = yield* seed()
        const tool = yield* TaskTool
        ...
        expect(result.output).toContain("Tool bash failed: command failed")
      }),
    ),
  )
