...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "codeplane.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "codeplane.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codeplane.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codeplane.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["codeplane.json", "codeplane.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "codeplane.json"),
        path.join(tmp.path, "codeplane.jsonc"),
        path.join(project, "codeplane.json"),
        path.join(project, "codeplane.jsonc"),
      ])
    })
  })
