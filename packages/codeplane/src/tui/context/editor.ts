    ...
    const connect = () => {
      ...
      current.addEventListener("open", () => {
        ...
        request("initialize", {
          ...
          capabilities: {},
          clientInfo: { name: "codeplane", version: "0.0.0" },
        })
      })
...
function resolveEditorConnection(directory: string): EditorConnection | undefined {
  const port = parsePort(process.env.CLAUDE_CODE_SSE_PORT || process.env.CODEPLANE_EDITOR_SSE_PORT)
  if (port) {
    return {
...
function openEditorSocket(connection: EditorConnection, WebSocketImpl: typeof WebSocket) {
  ...
  // The browser WebSocket constructor only accepts (url, protocols). Bun and
  // Node `ws` accept an options bag (headers, agent, ...). We're running
  // under Bun, so cast the constructor to the wider Bun-style signature.
  type BunWsCtor = new (url: string, opts?: { headers?: Record<string, string> }) => WebSocket
  const Ctor = WebSocketImpl as unknown as BunWsCtor
  return new Ctor(connection.url, {
    headers: {
