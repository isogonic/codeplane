// Wired against the augmented Event union from `@/tui/_compat/sdk-v2`
// (adds `id: string` to every variant and the 4 missing `session.next.*`
      ...
      session: {
        message: {
          async sync(sessionID: string) {
            // Their newer SDK exposes a `v2.session.messages` namespace and
            // returns `{ items: SessionMessage[] }`; ours returns the raw
            // array `[{ info, parts }, ...]` directly under `data`. Both come
            // from the same wire endpoint.
            type SessionMessagesV2 = { items: SessionMessage[] }
            const response = await sdk.client.session.messages({ sessionID })
            const v2 = response.data as unknown as SessionMessagesV2 | undefined
            setStore("messages", sessionID, reconcile(v2?.items ?? []))
          },
