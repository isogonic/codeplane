...
import {
  ...
  type PluginPackage,
  type PluginSource,
} from "@/tui/_compat/plugin-shared"
import { PluginLoader } from "@/tui/_compat/plugin-loader"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir = path.join(source_dir, "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".codeplane", "tui.json"),
  }
}
...
async function installPluginBySpec(
  ...
  // Cast against the core PluginKind ("server") so a literal "tui" check
  // typechecks. The plugin manifest carries any PluginKind value at runtime;
  // the type narrowing here is purely a compile-time relaxation.
  const tui = manifest.targets.find((item) => (item.kind as string) === "tui")
  if (tui) {
    const file = patch.items.find((item) => (item.kind as string) === "tui")?.file
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await WithInstance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.CODEPLANE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.CODEPLANE_PURE && config.plugin_origins?.length) {
