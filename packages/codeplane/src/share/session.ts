...
export class Service extends Context.Service<Service, Interface>()("@codeplane/SessionShare") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const share = Effect.fn("SessionShare.share")(function* (sessionID: SessionID) {
      ...
      // Typed failure, not a raw throw: inside an Effect.fn generator a thrown
      // Error becomes an unhandled defect, so callers can't catch it cleanly.
    ...
    const create = Effect.fn("SessionShare.create")(function* (input?: Session.CreateInput) {
      ...
      const conf = yield* cfg.get()
      if (!(Flag.CODEPLANE_AUTO_SHARE || conf.share === "auto")) return result
