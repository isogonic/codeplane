...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall codeplane and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# codeplane") || content.includes(".codeplane/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".codeplane/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".codeplane/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".codeplane"))
    ) {
      continue
    }
