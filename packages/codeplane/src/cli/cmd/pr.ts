...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run codeplane",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for codeplane session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found codeplane session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["codeplane", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting codeplane...")
        UI.println()

        const codeplaneArgs = sessionId ? ["-s", sessionId] : []
        const codeplaneProcess = Process.spawn(["codeplane", ...codeplaneArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await codeplaneProcess.exited
        if (code !== 0) throw new Error(`codeplane exited with code ${code}`)
      },
    })
  },
})
