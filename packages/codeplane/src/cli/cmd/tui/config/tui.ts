...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.codeplane`
  ...
  // 2. Explicit CODEPLANE_TUI_CONFIG override, if set.
  if (Flag.CODEPLANE_TUI_CONFIG) {
    const configFile = Flag.CODEPLANE_TUI_CONFIG
  ...
  // 4. `.codeplane` directories (and CODEPLANE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".codeplane") || dir === Flag.CODEPLANE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".codeplane") && dir !== Flag.CODEPLANE_CONFIG_DIR) continue
