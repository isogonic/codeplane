...
const TUI_SCHEMA_URL = "https://codeplane.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from codeplane.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const codeplane = await codeplaneFiles(input)
  for (const file of codeplane) {
...
async function codeplaneFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "codeplane"),
