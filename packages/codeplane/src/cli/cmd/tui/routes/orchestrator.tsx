...
import type { Message, Part, ToolPart } from "@codeplane-ai/sdk/v2"
...
function taskAgent(part: ToolPart) {
  const value = part.state.status !== "pending" ? part.state.metadata?.agent : undefined
  return Locale.titlecase(typeof value === "string" && value ? value : inputText(part, "subagent_type", "general"))
}

export function taskStatus(part: ToolPart) {
  const value = part.state.status !== "pending" ? part.state.metadata?.status : undefined
...
function taskDuration(part: ToolPart): string | undefined {
  if (part.state.status === "pending") return
  const startedAt = part.state.metadata?.startedAt
  const completedAt = part.state.metadata?.completedAt
  const start = typeof startedAt === "number" ? startedAt : part.state.time.start
  const end =
    taskStatus(part) === "running"
      ? Date.now()
...
export function OrchestratorView(props: { sessionID: string }) {
  ...
  const active = createMemo(() => tasks().filter((part) => taskStatus(part) === "running").length)
  const completed = createMemo(() => tasks().filter((part) => taskStatus(part) === "completed").length)
  const failed = createMemo(() => tasks().filter((part) => taskStatus(part) === "error").length)
...
function TaskNode(props: { part: ToolPart; depth: number; last: boolean; path: string[] }) {
  ...
  const color = createMemo(() => {
    const status = taskStatus(props.part)
