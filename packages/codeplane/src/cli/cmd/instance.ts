...
export function composeRemoteHeaders(input: InstanceAddArgs): Record<string, string> | undefined {
  ...
  if (user.includes(":")) throw new Error("HTTP Basic Auth username cannot contain a colon.")
  if (user || pass) {
...
export function parseInstanceHeaders(input: string[] = []) {
  return input.reduce<Record<string, string>>((result, item) => {
    ...
    if (key.length > 128) throw new Error(`Invalid header "${item}". Header names cannot exceed 128 characters.`)
    if (value.length > 8192) throw new Error(`Invalid header "${item}". Header values cannot exceed 8192 characters.`)
...
export function formatLocalVersions(
  ...
  return formatJson({
    ...
    ...(shownVersions.at(-1) ? { oldestShownVersion: shownVersions.at(-1) } : {}),
...
export function formatLocalStatus(status: LocalStatus & { target?: LocalTarget }, pathOnly?: boolean, installedOnly?: boolean, versionOnly?: boolean) {
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("version-only", {
        type: "boolean",
        default: false,
        describe: "print only the resolved local runtime version",
      }),
  async handler(args) {
