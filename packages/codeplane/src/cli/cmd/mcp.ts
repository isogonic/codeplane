    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: codeplane mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in codeplane.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .codeplane/ subdirectory too)
  const candidates = [path.join(baseDir, "codeplane.json"), path.join(baseDir, "codeplane.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".codeplane", "codeplane.json"), path.join(baseDir, ".codeplane", "codeplane.jsonc"))
  }
  ...
  // Default to codeplane.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., codeplane x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "codeplane-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
