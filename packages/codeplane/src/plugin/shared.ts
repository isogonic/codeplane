...
export async function checkPluginCompatibility(target: string, codeplaneVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(codeplaneVersion) || semver.major(codeplaneVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.codeplane
  if (typeof range !== "string") return
  if (!semver.satisfies(codeplaneVersion, range)) {
    throw new Error(`Plugin requires codeplane ${range} but running ${codeplaneVersion}`)
  }
}

export async function resolvePluginTarget(spec: string, dir?: string) {
