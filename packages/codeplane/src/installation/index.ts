...
export const USER_AGENT = `codeplane/${InstallationChannel}/${InstallationVersion}/${Flag.CODEPLANE_CLIENT}`
    ...
    Effect.gen(function* () {
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          const response = yield* httpOk.execute(HttpClientRequest.get("https://codeplane.ai/install"))
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "brew", command: () => text(["brew", "list", "--formula", "codeplane"]) },
          { name: "scoop", command: () => text(["scoop", "list", "codeplane"]) },
          { name: "choco", command: () => text(["choco", "list", "--limit-output", "codeplane"]) },
        ]
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "choco") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27codeplane%27%20and%20IsLatestVersion&$select=Version",
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/codeplane.json",
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "choco":
            result = yield* run(["choco", "upgrade", "codeplane", `--version=${target}`, "-y"])
            break
          case "scoop":
            result = yield* run(["scoop", "install", `codeplane@${target}`])
            break
          default:
