    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CODEPLANE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.CODEPLANE_CONFIG) {
          yield* merge(Flag.CODEPLANE_CONFIG, yield* loadFile(Flag.CODEPLANE_CONFIG))
          log.debug("loaded custom config", { path: Flag.CODEPLANE_CONFIG })
        }

        if (!Flag.CODEPLANE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("codeplane", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.CODEPLANE_CONFIG_DIR) {
          log.debug("loading config from CODEPLANE_CONFIG_DIR", { path: Flag.CODEPLANE_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.codeplane/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.CODEPLANE_CONFIG_CONTENT) {
          const source = "CODEPLANE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CODEPLANE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from CODEPLANE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CODEPLANE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CODEPLANE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CODEPLANE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CODEPLANE_DISABLE_PRUNE) {
