  ...
  const finishRun: (
    ...
    SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (st) {
        ...
        if (st.run.next && Exit.isSuccess(exit)) {
          const run = yield* startRun(st.run.next.work, st.run.next.done)
        ...
        return [
          Effect.gen(function* () {
            ...
            if (st.run.next) yield* complete(st.run.next.done, exit)
          }),
          { _tag: "Idle" } as const,
        ] as const
      }),
      ...
      Effect.fnUntraced(function* (st) {
        switch (st._tag) {
          case "Running": {
            ...
            if (st.run.next) return [Deferred.await(st.run.next.done), st] as const
            const run = {
              id: next(),
              done: yield* Deferred.make<A, E | Cancelled>(),
              work,
            } satisfies PendingHandle<A, E>
            return [Deferred.await(run.done), { _tag: "Running", run: { ...st.run, next: run } }] as const
          }
          case "ShellThenRun":
  ...
  const cancel = SynchronizedRef.modify(ref, (st) => {
    switch (st._tag) {
      ...
      case "Running":
        return [
          Effect.gen(function* () {
            if (st.run.next) yield* Deferred.fail(st.run.next.done, new Cancelled()).pipe(Effect.asVoid)
