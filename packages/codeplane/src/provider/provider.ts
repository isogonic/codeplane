...
function googleVertexAnthropicBaseURL(project: string | undefined, location: string | undefined) {
  ...
  return [
    ...
    `projects/${project}/locations/${location}/publishers/anthropic/models`,
  ].join("/")
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    codeplane: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["codeplane"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from codeplane.json provider config
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `codeplane/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `codeplane/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `codeplane auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `codeplane/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "codeplane",
          },
        },
      }),
    kilo: () =>
...
export function catalogProvider(provider: Info): Info {
  return {
    ...provider,
    models: Object.fromEntries(
      Object.entries(provider.models).filter(([modelID, model]) => {
        if (modelID === "gpt-5-chat-latest") return false
        if (provider.id === ProviderID.openrouter && modelID === "openai/gpt-5-chat") return false
        if (model.status === "alpha" && !Flag.CODEPLANE_ENABLE_EXPERIMENTAL_MODELS) return false
...
export class Service extends Context.Service<Service, Interface>()("@codeplane/Provider") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            if (model.status === "alpha" && !Flag.CODEPLANE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      if (providerID.startsWith("codeplane")) {
        priority = ["gpt-5-nano"]
      }
