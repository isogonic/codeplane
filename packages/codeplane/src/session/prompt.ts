...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const insertReminders = Effect.fn("SessionPrompt.insertReminders")(function* (input: {
      ...
      if (!Flag.CODEPLANE_EXPERIMENTAL_PLAN_MODE) {
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
              ...
              if (part.state.status === "running") {
                yield* sessions.updatePart({
                  ...
                  state: {
                    ...
                    metadata: { ...(part.state.metadata ?? {}), status: "error", completedAt: end },
          ...
          state: {
            ...
            metadata:
              part.state.status === "pending"
                ? undefined
                : { ...(part.state.metadata ?? {}), status: "error", completedAt: end },
            input: part.state.input,
          },
        } satisfies MessageV2.ToolPart)
      }
      ...
      yield* Effect.forEach(
        ...
        Effect.fnUntraced(function* (msg) {
          yield* Effect.forEach(
            msg.parts,
            (part) => {
              if (part.type === "text" && part.time && !part.time.end) {
                return sessions.updatePart({
                  ...part,
                  time: { ...part.time, end: completed },
                })
              }
              ...
              if (part.type === "tool" && (part.state.status === "pending" || part.state.status === "running")) {
                ...
                return sessions.updatePart({
                  ...
                  state: {
                    ...
                    time: {
                      start: part.state.status === "running" ? part.state.time.start : completed,
                      end: completed,
                    },
                  },
                })
    ...
    const collectLoopState = (msgs: MessageV2.WithParts[]) => {
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          if (task?.type === "subtask") {
            ...
            const end = pendingTasks.findIndex((item) => item.type !== "subtask" || !!item.command !== isCommand)
