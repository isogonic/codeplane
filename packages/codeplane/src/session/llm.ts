...
export class Service extends Context.Service<Service, Interface>()("@codeplane/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via codeplane's tool system
      ...
      return streamText({
        ...
        headers: {
          ...(input.model.providerID.startsWith("codeplane")
            ? {
                "x-codeplane-project": Instance.project.id,
                "x-codeplane-session": input.sessionID,
                "x-codeplane-request": input.user.id,
                "x-codeplane-client": Flag.CODEPLANE_CLIENT,
              }
            : {
