...
function parseField(spec: string, min: number, max: number, names?: Record<string, number>): Set<number> {
  ...
  for (const part of spec.split(",")) {
    ...
    if (rangeStr === "*") {
      ...
      start = parseValue(a, min, max, names)
      end = parseValue(b, min, max, names)
    } else {
...
export function parse(expression: string): ParsedExpression {
  ...
  return {
    ...
    dayOfWeek: parseField(tokens[4], FIELDS[4].min, FIELDS[4].max, DAY_NAMES),
  }
}
...
export function next(expression: string, from: Date | number = Date.now()): number {
  ...
  throw new CronParseError(`Could not find next time within bound for ${expression}`)
}
