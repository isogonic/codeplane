...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for codeplane.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal codeplane sessions
...
codeplane acp --cwd /path/to/project
```
...
```bash
CODEPLANE_ENABLE_QUESTION_TOOL=1 codeplane acp
```
...
- **Terminal Integration**: Full terminal support via codeplane's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | codeplane acp
```
