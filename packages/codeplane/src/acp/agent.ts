...
import type { AssistantMessage, Event, CodeplaneClient, SessionMessageResponse, ToolPart } from "@codeplane-ai/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: CodeplaneClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `codeplane auth login` in the terminal",
      name: "Login with codeplane",
      id: "codeplane-login",
    }
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const codeplaneProvider = providers.find((p) => p.id === "codeplane")
  if (codeplaneProvider) {
    if (codeplaneProvider.models["big-pickle"]) {
      return { providerID: ProviderID.codeplane, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(codeplaneProvider.models))
    if (best) {
      return {
