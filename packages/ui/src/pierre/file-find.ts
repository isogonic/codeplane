...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("codeplane-find")
  api.delete("codeplane-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("codeplane-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("codeplane-find", new Highlight(...rest))
    return true
  }
