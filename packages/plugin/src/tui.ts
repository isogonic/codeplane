import type {
  AgentPart,
  CodeplaneClient as OpencodeClient,
  Event,
  FilePart,
  LspStatus,
  McpStatus,
...
// PluginConfig in our codeplane plugin pkg doesn't yet model `theme`/`keybinds`/
// `tui` fields the way their TUI expects. Cast the lookup to a relaxed shape
// here so this barrel compiles; the runtime objects always carry the values.
type _RelaxedPluginConfig = PluginConfig & {
  $schema?: string
  theme?: unknown
  keybinds?: unknown
  plugin?: unknown
...
type TuiConfigView = Pick<_RelaxedPluginConfig, "$schema" | "theme" | "keybinds" | "plugin"> &
  NonNullable<_RelaxedPluginConfig["tui"]> & {
...
export type TuiEventBus = {
  // Accepts both SDK event types and TUI-local event types defined via
  // `BusEvent.define` (toast.show, command.execute, prompt.append, etc.).
  // Extract<Event, ...> still narrows for SDK-known event names; otherwise
  // the handler receives the raw payload structure.
  on: <Type extends string>(
    type: Type,
    handler: (event: Type extends Event["type"] ? Extract<Event, { type: Type }> : { type: Type; properties: Record<string, unknown> }) => void,
  ) => () => void
}
