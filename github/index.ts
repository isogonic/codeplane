...
const { client, server } = createCodeplane()
...
try {
  ...
  await assertCodeplaneConnected()
  ...
  console.log("codeplane session", session.id)
  if (shareId) {
...
function createCodeplane() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`codeplane`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createCodeplaneClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/codeplane|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/codeplane` or `/oc`")
  }
}
...
async function assertCodeplaneConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to codeplane server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.codeplane.ai" : "https://codeplane.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.codeplane.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("codeplane-github-action")
    response = await fetch("https://api.codeplane.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/codeplane" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/codeplane") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to codeplane...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "codeplane-agent[bot]"`
  await $`git config --global user.email "codeplane-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `codeplane/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/codeplane-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[codeplane session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
